...
| J3 | v1793 | local gates passed; remote CI pending | Added `application-prod.yml` with H2 console and SQL debug output disabled, graceful shutdown enabled, and health probes retained; CI prod profile boot smoke starts the packaged jar and checks `/actuator/health`; `compose.yaml` and `docker-compose.yml` now use env substitution plus `.env.example`; order and failed-event write request DTOs now have Bean Validation boundaries; focused prod/config/order/failed-event validation tests, walkthrough compliance, Spotless ratchet check, default verify, Java 21 prod smoke, and docker profile verify passed |
| J4 | — | not started | |
| J5 | — | not started | |
