...
Status: v1899 release lifecycle is complete. External Java-track review is required before the
...
| --- | --- | --- | --- |
| E1 Build & CI | Parent `.github/workflows/maven-ci.yml`; Maven wrapper; headless and optional Docker jobs | `JavaTrackCloseoutTests.workflowUsesCurrentActions`; real Actions runs | v1899 local gate plus implementation `30019562919` and closeout `30021301566` green |
...
| E8 Release discipline | `CHANGELOG.md`, git-tag policy, progress ledger, implementation/closeout commits | `JavaTrackCloseoutTests.docsAndReleaseStayHonest`; external git/CI check | v1899 tag peels to green closeout `7c171c6c` locally and remotely |
| E9 Code health | root 104/104/0, no Java file above 750 lines, route owner 27 fields/69 lines, exact name baseline plus staged change gate | extraction, maintainability, elegance, and HTTP-boundary ratchets | v1899 focused gates: ops 1,161; production max/over500 658/29; production/test names 1037/19155/2589 and 680/9755/3633 |
| E10 Archive retention | exact SHA-256 manifest, count/raw-byte ceilings, frozen archive policy; CRLF is canonicalized to LF for text only | `ArchiveRetentionTests` and `archive-retention-census.ps1` | v1899 exact candidate set 1,710 files / 20,374,361 raw bytes; focused gate green |

## Final Censuses

- Ops direct root: 104 files; retained: 104; remaining movable: 0; unassigned: 0.
- Ops total production Java files: 1,161; no extraction family remains in root.
- Production Java: 1,293 files; maximum 658 lines; over 500: 29; over 750/1000: 0/0.
- Test Java: 912 files; maximum 699 lines; over 500: 8;
  over 750/1000: 0/0.
- Long-name shrink-only baseline: production stems/uses/unique 1037/19155/2589;
  tests 680/9755/3633. New declarations and filenames remain within 40 characters;
  baseline identities and aggregate occurrences only shrink during staged migration.
- Catalog census: 243 files, down from 332 at v1886. CandidateDocument request and
  ...
  the release-acceptance sustainment package replaces a further nine with one typed snapshot.
- Renderer census: 30 files / 3,176 lines / 0 long filenames, down from
...
- Archive retention: every authorized optimization adds exactly its named walkthrough;
  the current exact candidate set is 1,710 files / 20,374,361 raw bytes.
...
owners and the 394-line long-name Support become one 299-line `DossierCatalog`, one immutable
typed Evidence, and one 226-line `DossierSupport`. Service assembles once while Renderer and
Support read the same snapshot. Public route, Response, Controller, ordering, Markdown, 34 checks,
and read-only transaction remain unchanged.

The version also replaces repeated predicate-count helpers in six archive, digest, package, and
dossier callers with the 12-line `EvidenceCounts` owner. Production Java reaches 1,316 files, ops
1,184, Catalogs 266, the dossier package 5, and tests 906. Production/test name metrics reach
`1073/19646/2632` and `694/9780/3655`; the exact baseline has 46 removals and no additions.
Readiness reaches 985 and renderer lines 3,209. Focused behavior, oracle, shared-count, structure,
change, and elegance gates pass. The walkthrough has 3,676 Han and ten standard headings; the
exact archive is 1,706 files / 20,300,596 raw bytes. The first full local release gate exposes the
stale v1866 two-file evidencecore inventory; the strengthened gate
then requires three explicit owners and constrains `EvidenceCounts` to public/final/below 20 lines.
The clean rerun pins v1894 `105271fb`, passes 2,023 tests in 15:15, analyzes 2,079 JaCoCo classes
with every floor met, reports SpotBugs 0/0, and packages a 67,966,474-byte jar. Implementation
...
Current v1896 candidate: the complete sandbox blocked-execution dossier first passes its
canonical JSON oracle on released v1895. The collection vector is
`1/3/5/6/5/12/4/5/10/4/9/21` and the full sorted-property UTF-8 SHA-256 is
`f4ff835d241fd99fd1113f926f542c6954ab22f409ff43ef78b6e34f4413fad2`. Nine one-list
Catalog owners and the 328-line long-name Support become one 374-line `DossierCatalog`, one
immutable typed Evidence, and one 173-line `DossierSupport`. Service assembles once while
Renderer and Support read the same snapshot. Public route, Response, Controller, ordering,
nine Markdown sections, 21 checks, read-only transaction, and Java/mini-kv execution denials
remain unchanged.

The family contracts from 13 to 5 production files and from 1,039 to 740 lines. Production
Java reaches 1,308 files, ops 1,176, Catalogs 258, and tests 907. Production/test name metrics
reach `1063/19545/2622` and `690/9773/3651`; the exact baseline has 28 removals and no
additions. Readiness reaches 975 and renderer lines 3,203. Focused behavior, complete-oracle,
history, structure, census, and elegance gates pass 52/52. The walkthrough has 3,097 Han and
ten standard headings; the exact archive is 1,707 files / 20,316,569 raw bytes.
The release gate first removed a stale waiver for the retired precondition Catalog, then the real
SpotBugs scan exposed default-locale case folding in `evidenceId`. `Locale.ROOT` plus a Turkish
locale regression fixes the root cause; the exact waiver set tightens from 675 to 674 without
...
0 bugs / 0 errors, and the packaged jar is 67,950,901 bytes. Implementation
`010e4e43488017971f297d55933ea38bac8f1c34` passes canonical run `30001832600`:
Docker completes in 2:06 with a 1:51 wrapper verify; headless completes in 18:16 with a
17:33 wrapper verify, 0:11 production-profile smoke, and 0:03 JaCoCo upload. Closeout
`fd803c13889e90b948c313207713252413bf09fb` passes run `30003161206`: Docker-tagged
verification completes in 2:13 with a 1:56 wrapper verify; headless regression completes in
17:56 with a 17:22 wrapper verify, 0:10 production-profile smoke, and 0:03 JaCoCo upload.
...
Catalog owners and the 420-line long-name Support become one 330-line `PackageCatalog`, one
immutable typed Evidence, and one 219-line `PackageSupport`. Service assembles once while the
130-line Renderer and Support read the same snapshot. Public route, Response, Controller,
ordering, nine Markdown sections, 40 checks, and read-only transaction remain unchanged. The
independent closeout receipt and closeout archive-index subfamilies are outside this cut.

The main family contracts from 13 to 5 production files and from 1,167 to 773 lines. Production
Java reaches 1,293 files, ops 1,161, Catalogs 243, and tests 909. Production/test name metrics
reach `1044/19346/2603` and `680/9763/3641`; the exact baseline has 30 removals and no
additions. Readiness reaches 956 and renderer lines 3,176. Focused complete-oracle, behavior,
history, structure, elegance, name, and readability gates pass. The walkthrough has 3,177 Han
and ten standard headings; the exact archive candidate is 1,709 files / 20,354,150 raw bytes.
The first full gate reaches all 2,032 tests and exposes only the exact three-label design-note
protocol. The existing design note receives the required `Abstraction`, `Data boundary`, and
`Behavior boundary` labels without changing its content or the test; the 30/30 repair selection
passes. The clean full rerun pins v1897 closeout
`fd803c13889e90b948c313207713252413bf09fb`, passes 2,032 tests in 9:22, analyzes 2,059
JaCoCo classes with every floor met, reports SpotBugs 0/0, and packages a 67,941,575-byte jar.
...
Released v1899: three consecutive release-approval marker builders first pass two
complete response oracles on released v1898. After removing only the request-time
`sampledAt` and `serverTimestamp` fields, the default response SHA-256 is
`48dc64dd2385de0ad0b98f114be157c98b19012abcfde8384ff6e237248b8550`; the complete
header-backed response is
`c64e2fac8194ab2f70ef5bbd603a9a92dd0ea1a9ae75459f386c7fa6373258cc`.
The old implementation passes both snapshots twice before replacement.

The 738/726/590-line package-private monoliths become three short builder/rules pairs plus
26-line immutable `MarkerEvidence`. Shared metadata transport is centralized while typed
boundary projection and fail-closed predicates remain in their domains. Public route,
Request/Response FQNs, nested records, JSON, marker digests, list order, warnings, proof claims,
Node actions, and all denied execution capabilities remain unchanged.

Four terminal 58-line receipt wrappers that owned no state or rule also retire; typed Support
methods now serve their build, contribution, warning-digest, and no-write consumers directly.
The complete response oracle covers the whole tail after this indirection removal.

The `releaseapproval` package stays at 119 files and tightens from 38,879 to 38,570 lines.
Production Java stays 1,293, ops stays 1,161, and tests reach 912; Catalogs remain 243,
Services 375, and Renderers 30 / 3,176 lines / 0 long filenames. Production max/over-500
tightens from 738/32 to 658/29. Production/test name metrics reach `1037/19155/2589` and
`680/9755/3633`; the exact baseline removes 22 entries and adds none. Focused oracle,
snapshot, structure, history, warning-digest, elegance, walkthrough, and archive gates pass.
The walkthrough has 3,803 Han and ten standard headings; the exact archive candidate is
1,710 files / 20,374,361 raw bytes. The full release gate peels the v1898 predecessor to
`78cac4e90557b599692608a40dce183e94c42424`, passes 2,038 tests in 9:21, analyzes
2,059 JaCoCo classes with every floor met, reports SpotBugs 0/0, passes production smoke,
and packages a 67,940,843-byte jar. Remote success must not be inferred from local evidence.
Implementation
`f1980b416c9e3258eda033c7da55e9fa05e67525` passes canonical run `30019562919`:
Docker-tagged verification completes in 2:02 with a 1:52 wrapper verify; headless regression
completes in 19:07 with an 18:22 wrapper verify, 0:12 production-profile smoke, and 0:03
JaCoCo upload. Closeout `7c171c6ccae2b7b037a7224cc98a6a2537aad416` passes run
`30021301566`: Docker-tagged verification completes in 1:56 with a 1:46 wrapper verify;
headless regression completes in 16:16 with a 15:31 wrapper verify, 0:11 production-profile
smoke, and 0:03 JaCoCo upload. Annotated tag
`v1899-order-platform-marker-evidence-builders` peels to the closeout locally and remotely.
