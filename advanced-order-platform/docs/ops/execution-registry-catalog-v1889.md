...
- 完整 `scripts/verify-release.ps1` 固定 v1888 commit `15ad48bd`，通过 2,007 个测试，
  ...
  全绿：Docker 2:04、headless 19:10，包含 prod smoke 与 JaCoCo 上传。Closeout
  `99e1afd2` 的 run `29884385641` 通过 Docker 2:19 与 headless 19:14；annotated tag
  `v1889-order-platform-execution-registry-catalog` 在本地和 `javaproject` 均 peel 到该 closeout。
