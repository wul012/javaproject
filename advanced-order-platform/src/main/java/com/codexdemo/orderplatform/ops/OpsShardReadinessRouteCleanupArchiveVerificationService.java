...
public class OpsShardReadinessRouteCleanupArchiveVerificationService {
    ...
    public OpsShardReadinessRouteCleanupArchiveVerificationResponse verification() {
        ...
        List<OpsShardReadinessRouteCleanupArchiveVerificationResponse.VerificationCheck> checks = List.of(
                check("archive-plan-passed", archivePlan.status().equals("passed"), archivePlan.archiveProfile()),
                check("archive-artifacts-required", archivePlan.artifacts().stream().allMatch(
                        OpsShardReadinessRouteCleanupArchivePlanResponse.ArchiveArtifact::required
                ), "required artifacts=" + archivePlan.artifactCount()),
                check("closeout-passed", closeout.status().equals("passed"), closeout.closeoutProfile()),
                check("digest-present", closeout.digestValue().matches("[0-9a-f]{64}"), closeout.digestValue())
        );
        boolean passed = checks.stream()
                .allMatch(OpsShardReadinessRouteCleanupArchiveVerificationResponse.VerificationCheck::passed);
