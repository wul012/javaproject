...
public class OpsShardReadinessRouteCleanupSourcePlanAlignmentService {
    ...
    public OpsShardReadinessRouteCleanupSourcePlanAlignmentResponse alignment() {
        List<OpsShardReadinessRouteCleanupSourcePlanAlignmentResponse.AlignmentItem> alignments = List.of(
                item("source-plan", SOURCE_PLAN, SOURCE_PLAN, true),
                item("sibling-startup", "Java and mini-kv do not need to start", "not started by Java suite", true),
                item("runtime-boundary", "executionAllowed=false", "executionAllowed=false", true),
                item("collaboration-mode", "Java remains recommended parallel", "read-only Java evidence suite", true)
        );
        return new OpsShardReadinessRouteCleanupSourcePlanAlignmentResponse(
                ...
                alignments,
                alignments.stream()
                        .allMatch(OpsShardReadinessRouteCleanupSourcePlanAlignmentResponse.AlignmentItem::aligned)
                        ? "passed"
                        : "blocked"
        );
    }
