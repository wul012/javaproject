...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutItemCatalog {
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutResponse.CloseoutItem> items(
        ...
        return List.of(
                item("stable-barrel-preserved", source.routePaths().stream()
                        .allMatch(OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse
                                .RoutePathEntry::legacyCompatible)),
                item("compatibility-catalog-extracted", source.compatibilityChecks().stream()
                        .allMatch(OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse
                ...
                item("node-v1866-parallel-no-fresh-evidence", "Node v1847-v1866"
                        .equals(OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutSupport.NODE_PARALLEL_PLAN)),
                item("future-route-owner-rule", source.consumerHandoffs().stream()
                        .anyMatch(handoff -> handoff.consumer().equals("future-release-acceptance-services")))
        );
    }
