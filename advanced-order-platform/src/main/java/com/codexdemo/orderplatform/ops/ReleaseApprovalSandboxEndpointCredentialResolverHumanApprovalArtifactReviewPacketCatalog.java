...
final class ReleaseApprovalSandboxEndpointCredentialResolverHumanApprovalArtifactReviewPacketCatalog {

    private static final List<RequiredFieldTemplate> REQUIRED_FIELD_TEMPLATES = List.of(
            requiredField("artifact_id", "Artifact id",
                    "Stable artifact id bound to the human review packet."),
            requiredField("operator_approval_reference", "Operator approval reference",
                    "Human approval reference or ticket handle."),
            requiredField("credential_handle_review_status", "Credential handle review status",
            ...
            requiredField("no_network_safety_test_reference", "No-network safety test reference",
                    "Evidence that review packet validation performs no HTTP/TCP request."),
            requiredField("manual_abort_semantics_reference", "Manual abort semantics reference",
                    "Documented operator abort behavior before runtime discussion."),
            requiredField("rollback_semantics_reference", "Rollback semantics reference",
                    "Documented rollback behavior without executing rollback."),
            requiredField("created_by_operator_identity", "Created by operator identity",
                    "Verified operator identity for the review artifact."),
            requiredField("audit_correlation_id", "Audit correlation id",
    ...
    private static final List<ProhibitedFieldTemplate> PROHIBITED_FIELD_TEMPLATES = List.of(
            ...
            prohibitedField("runtime_shell_invocation_request",
    ...
    private static final List<RejectionReasonTemplate> REJECTION_REASON_TEMPLATES = List.of(
            ...
            rejectionReason("MISSING_OPERATOR_APPROVAL_REFERENCE",
                    "Reject when the operator approval reference is absent."),
            rejectionReason("MISSING_CREDENTIAL_HANDLE_REVIEW_STATUS",
                    "Reject when credential handle review status is absent."),
            rejectionReason("MISSING_ENDPOINT_HANDLE_ALLOWLIST_REVIEW_STATUS",
                    "Reject when endpoint handle allowlist review status is absent."),
            rejectionReason("MISSING_NO_NETWORK_SAFETY_TEST_REFERENCE",
                    "Reject when no-network safety test reference is absent."),
            rejectionReason("MISSING_ABORT_OR_ROLLBACK_SEMANTICS",
            ...
            rejectionReason("EXTERNAL_REQUEST_REQUESTED",
                    "Reject external request payloads or execution requests."),
            rejectionReason("WRITE_OR_SCHEMA_MUTATION_REQUESTED",
                    "Reject ledger writes and schema migration requests."),
            rejectionReason("MINI_KV_WRITE_OR_AUTHORITY_REQUESTED",
                    "Reject mini-kv writes, admin commands, and authority claims."),
            rejectionReason("RUNTIME_SHELL_IMPLEMENTATION_REQUESTED",
    ...
    private static final List<String> NODE_WARNING_CODES = List.of(
            "REVIEW_PACKET_DOES_NOT_AUTHORIZE_RUNTIME_SHELL"
    );
    ...
    private ReleaseApprovalSandboxEndpointCredentialResolverHumanApprovalArtifactReviewPacketCatalog() {
    }

    static List<RehearsalHumanApprovalArtifactReviewRequiredField> requiredFields() {
        return REQUIRED_FIELD_TEMPLATES.stream()
                .map(template -> new RehearsalHumanApprovalArtifactReviewRequiredField(
                        template.id(),
                        template.label(),
    ...
    static List<RehearsalHumanApprovalArtifactReviewProhibitedField> prohibitedFields() {
        return PROHIBITED_FIELD_TEMPLATES.stream()
                .map(template -> new RehearsalHumanApprovalArtifactReviewProhibitedField(
                        template.id(),
                        template.reason(),
    ...
    static List<RehearsalHumanApprovalArtifactReviewRejectionReason> rejectionReasons() {
        return REJECTION_REASON_TEMPLATES.stream()
                .map(template -> new RehearsalHumanApprovalArtifactReviewRejectionReason(
                        template.code(),
                        template.message()
                ))
                .toList();
    ...
    static List<RehearsalHumanApprovalArtifactReviewMissingFieldCheck> missingFieldChecks(
        ...
        return requiredFields.stream()
                .map(field -> new RehearsalHumanApprovalArtifactReviewMissingFieldCheck(
                        field.id(),
                        field.missingFieldCode()
                ))
                .toList();
    }
    ...
    static List<RehearsalHumanApprovalArtifactReviewNoGoBoundary> noGoBoundaries() {
        return NO_GO_BOUNDARY_TEMPLATES.stream()
                .map(template -> new RehearsalHumanApprovalArtifactReviewNoGoBoundary(
                        template.id(),
                        true,
                        template.reason()
                ))
    ...
    private static RequiredFieldTemplate requiredField(String id, String label, String requiredEvidence) {
