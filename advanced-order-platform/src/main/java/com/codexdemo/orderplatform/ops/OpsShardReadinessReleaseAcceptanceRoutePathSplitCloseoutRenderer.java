...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutRenderer {
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutResponse.MarkdownSection> render(
        ...
        return List.of(
                section("Closeout Items", closeoutItems.stream()
                        .map(item -> "- " + item.item() + " passed=" + item.passed())
                        .toList()),
                section("Boundary Assertions", boundaryAssertions.stream()
                        .map(assertion -> "- " + assertion.boundary() + " locked=" + assertion.locked())
                        .toList()),
                section("Parallel Plan", List.of(
                        "- Node plan " + OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutSupport
                                .NODE_PARALLEL_PLAN,
    ...
    private static OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutResponse.MarkdownSection section(
            String heading,
            List<String> lines
    ) {
