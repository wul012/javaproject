...
public class OpsShardReadinessRouteCleanupMaintenanceArchiveDigestLedgerService {
    ...
    private String digest(OpsShardReadinessRouteCleanupMaintenanceUpkeepCatalog.Item item) {
        try {
            ...
            byte[] bytes = digest.digest((item.name() + "|" + item.endpoint() + "|" + item.evidencePath())
    ...
    private String status(
        ...
        boolean passed = entries.size() == 9
                && entries.stream().allMatch(entry -> entry.digest().length() == DIGEST_LENGTH)
                && entries.stream().map(OpsShardReadinessRouteCleanupMaintenanceArchiveDigestLedgerResponse
                        .LedgerEntry::digest).distinct().count() == entries.size()
