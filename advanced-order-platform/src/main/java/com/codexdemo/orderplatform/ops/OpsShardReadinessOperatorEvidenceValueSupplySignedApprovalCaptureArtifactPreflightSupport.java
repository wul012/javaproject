...
final class OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightSupport {
    ...
    private OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightSupport() {
    }

    static OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse response(
            String version,
            String endpoint,
            String profile,
            List<OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse
                    .ArtifactFragment> fragments,
            List<OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactSeal>
                    seals,
            List<OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactGate>
                    gates,
            List<String> additionalChecks
    ) {
        List<OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactFragment>
                fragmentCopy = List.copyOf(fragments);
        List<OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactSeal>
                sealCopy = List.copyOf(seals);
        List<OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactGate>
                gateCopy = List.copyOf(gates);
        int passedFragmentCount = (int) fragmentCopy.stream()
                .filter(fragment -> "passed".equals(fragment.status()))
                .count();
        int passedSealCount = (int) sealCopy.stream()
                .filter(seal -> "passed".equals(seal.status()))
                .count();
        List<String> checks = new ArrayList<>();
        checks.add("signed-approval-capture-artifact-preflight-fragment-count-" + fragmentCopy.size());
        checks.add("signed-approval-capture-artifact-preflight-passed-fragment-count-" + passedFragmentCount);
        ...
        checks.add("signed-approval-capture-artifact-preflight-source-capture-" + SOURCE_CAPTURE_PREFLIGHT_VERSION);
        checks.add("signed-approval-capture-artifact-preflight-source-java-" + SOURCE_JAVA_CAPTURE_PREFLIGHT_VERSION);
        checks.add("signed-approval-capture-artifact-preflight-source-template-" + SOURCE_TEMPLATE_VERSION);
        checks.add("signed-approval-capture-artifact-preflight-source-review-"
    ...
    static OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactFragment
    fragment(
            String code,
    ...
    static OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactSeal
    seal(
            String code,
            String category,
    ...
    static OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightResponse.ArtifactGate gate(
            String code,
            String category,
            String gate,
            String enforcement
