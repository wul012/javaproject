...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestSectionCatalog {
    ...
    static List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
        ...
        return List.of(
                section("source-handoff-snapshot", 1, "passed".equals(source.status()) ? 1 : 0,
                        source.sourceHandoffVersion()),
                section("artifact-verifications", source.artifactVerificationCount(),
                        source.passedArtifactVerificationCount(),
                        "artifacts=" + source.passedArtifactVerificationCount() + "/"
                                + source.artifactVerificationCount()),
                section("operator-lane-verifications", source.operatorLaneVerificationCount(),
                        source.passedOperatorLaneVerificationCount(),
                        "lanes=" + source.passedOperatorLaneVerificationCount() + "/"
                                + source.operatorLaneVerificationCount()),
                section("ci-batch-verifications", source.ciBatchVerificationCount(),
                        source.passedCiBatchVerificationCount(),
                        "ci-batches=" + source.passedCiBatchVerificationCount() + "/"
                                + source.ciBatchVerificationCount()),
                section("boundary-lock-verifications", source.boundaryVerificationCount(),
                        source.passedBoundaryVerificationCount(),
                        "boundaries=" + source.lockedBoundaryVerificationCount() + "/"
                                + source.boundaryVerificationCount()),
                section("source-archive-scorecard", source.scorecardEntryCount(),
