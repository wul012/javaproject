...
public class OpsShardReadinessRouteCleanupMaintenanceOperatorReviewPacketService {
    ...
    public OpsShardReadinessRouteCleanupMaintenanceOperatorReviewPacketResponse packet() {
        ...
        List<OpsShardReadinessRouteCleanupMaintenanceOperatorReviewPacketResponse.ReviewSection> sections = List.of(
                section("upkeep-catalog", catalog.profile(), catalog.endpoint(), catalog.itemCount(), catalog.status()),
                section("consumer-handoff-matrix", matrix.profile(), matrix.endpoint(),
                        matrix.matrixEntryCount(), matrix.status()),
                section("ci-expectation-manifest", ci.profile(), ci.endpoint(),
                        ci.expectationCount(), ci.status()),
                section("fail-closed-policy", policy.profile(), policy.endpoint(),
                        policy.policyCount(), policy.status()),
                section("archive-digest-ledger", ledger.profile(), ledger.endpoint(),
                        ledger.ledgerEntryCount(), ledger.status())
        );
    ...
    private OpsShardReadinessRouteCleanupMaintenanceOperatorReviewPacketResponse.ReviewSection section(
            String name,
    ...
    private String status(
            List<OpsShardReadinessRouteCleanupMaintenanceOperatorReviewPacketResponse.ReviewSection> sections
    ) {
        boolean passed = sections.size() == 5
