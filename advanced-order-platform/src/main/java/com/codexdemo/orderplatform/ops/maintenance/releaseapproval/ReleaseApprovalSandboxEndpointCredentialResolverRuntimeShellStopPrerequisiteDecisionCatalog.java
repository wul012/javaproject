...
final
class ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellStopPrerequisiteDecisionCatalog {

  private static final List<PrerequisiteTemplate> REQUIRED_PREREQUISITE_TEMPLATES =
      List.of(
  ...
  private static final List<NoGoTemplate> NO_GO_TEMPLATES =
      List.of(
          new NoGoTemplate(
  ...
  private static final List<String> NODE_WARNING_CODES =
      List.of("PREREQUISITE_DECISION_DOES_NOT_AUTHORIZE_RUNTIME");

  private static final List<String> NODE_RECOMMENDATION_CODES =
      List.of("RUN_JAVA_V141_AND_MINIKV_V134_IN_PARALLEL", "KEEP_RUNTIME_SHELL_BLOCKED");
  ...
  static List<String> requiredPrerequisiteIds() {
    return REQUIRED_PREREQUISITE_TEMPLATES.stream().map(PrerequisiteTemplate::id).toList();
  }
  ...
  static List<RehearsalRuntimeShellChainPrerequisite> requiredPrerequisites() {
    return REQUIRED_PREREQUISITE_TEMPLATES.stream()
        .map(
            template ->
                new RehearsalRuntimeShellChainPrerequisite(
                    template.id(),
                    template.label(),
  ...
  static List<String> noGoConditionCodes() {
    return NO_GO_TEMPLATES.stream().map(NoGoTemplate::code).toList();
  }
  ...
  static List<RehearsalRuntimeShellChainNoGoCondition> noGoConditions() {
    return NO_GO_TEMPLATES.stream()
        .map(
            template ->
                new RehearsalRuntimeShellChainNoGoCondition(
                    template.code(),
                    template.condition(),
