...
final class OpsShardReadinessReleaseAcceptanceArchiveVerificationHandoffRetentionCatalog {
    ...
    static List<OpsShardReadinessReleaseAcceptanceArchiveVerificationHandoffResponse.RetentionGuard> guards(
        ...
        return source.retentionWindows().stream()
                .map(window -> new OpsShardReadinessReleaseAcceptanceArchiveVerificationHandoffResponse.RetentionGuard(
                        window.name(),
