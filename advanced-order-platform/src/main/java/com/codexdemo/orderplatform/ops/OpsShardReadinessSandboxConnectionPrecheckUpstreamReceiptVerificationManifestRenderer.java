...
final class OpsShardReadinessSandboxConnectionPrecheckUpstreamReceiptVerificationManifestRenderer {
    ...
    static List<OpsShardReadinessSandboxConnectionPrecheckUpstreamReceiptVerificationManifestResponse.MarkdownSection>
    render(
        ...
        return List.of(
                section("Source Receipt", sourceReceipts.stream()
                        ...
                        .toList()),
                section("Split Modules", splitModules.stream()
                        .map(module -> module.version() + " " + module.moduleName())
                        .toList()),
                section("Evidence References", evidenceReferences.stream()
                        .map(reference -> reference.id() + " -> " + reference.version())
                        .toList()),
                section("Precheck Fields", precheckFields.stream()
                        .map(field -> field.id() + "=" + field.fieldName())
                        .toList()),
                section("Boundary Guards", boundaryGuards.stream()
                        .map(guard -> guard.name() + "=" + guard.passed())
                        .toList()),
                section("Code Health Gates", codeHealthGates.stream()
                        .map(gate -> gate.name() + "=" + gate.passed())
                        .toList()),
                section("Verification Gates", verificationGates.stream()
                        .map(gate -> gate.name() + "=" + gate.passed())
                        .toList()),
                section("Handoff Notes", handoffNotes.stream()
                        .map(note -> note.audience() + ": " + note.note())
                        .toList())
        );
    }

    private static OpsShardReadinessSandboxConnectionPrecheckUpstreamReceiptVerificationManifestResponse.MarkdownSection
