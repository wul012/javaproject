...
final class OpsShardReadinessSignedApprovalDraftTextPackageProfileSectionRegistrySupport {
  ...
  static List<String> gates() {
    return IntStream.rangeClosed(1, EXPECTED_GATE_COUNT)
        .mapToObj(
            index ->
                "signed-approval-draft-text-package-profile-section-registry-no-runtime-gate-"
                    + index)
        .toList();
  }
  ...
  static OpsShardReadinessSignedApprovalDraftTextPackageProfileSectionRegistryResponse response(
    ...
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-source-plan-" + SOURCE_PLAN);
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-source-node-"
            + SOURCE_NODE_TEXT_PACKAGE_RENDERER_VERSION);
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-module-count-"
            + moduleCopy.size());
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-source-route-count-"
            + sourceCopy.size());
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-section-count-"
            + sectionCopy.size());
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-submission-renderer-section-count-"
            + submissionCount);
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-compared-evidence-renderer-section-count-"
            + comparedEvidenceCount);
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-rendered-section-count-"
            + renderedCopy.size());
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-field-entry-count-"
            + fieldCopy.size());
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-route-field-lock-count-"
            + lockCopy.size());
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-locked-route-field-count-"
            + lockedRouteFieldCount);
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-gate-count-"
            + gateCopy.size());
    checks.add(
    ...
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-package-acceptance-disabled");
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-signature-capture-disabled");
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-approval-grant-disabled");
    ...
    checks.add(
        "signed-approval-draft-text-package-profile-section-registry-sibling-mutation-disabled");
  ...
  private static int rendererGroupCount(
    ...
    return (int)
        sections.stream().filter(section -> rendererGroup.equals(section.rendererGroup())).count();
  }
