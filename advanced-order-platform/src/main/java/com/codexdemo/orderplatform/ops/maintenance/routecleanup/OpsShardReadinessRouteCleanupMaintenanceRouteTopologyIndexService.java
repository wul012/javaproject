...
public class OpsShardReadinessRouteCleanupMaintenanceRouteTopologyIndexService {
  ...
  static final String ENDPOINT =
      RouteCleanupRoutes.BASE_PATH + RouteCleanupRoutes.MAINTENANCE_ROUTE_TOPOLOGY_INDEX;
  ...
  public OpsShardReadinessRouteCleanupMaintenanceRouteTopologyIndexResponse index() {
    ...
    List<OpsShardReadinessRouteCleanupMaintenanceRouteTopologyIndexResponse.RouteNode> routes =
        IntStream.range(0, items.size()).mapToObj(index -> node(items, index)).toList();
    List<String> checks =
        List.of(
  ...
  private String status(
    ...
    boolean passed =
        routes.size() == 9
            ...
            && routes.stream()
                .allMatch(route -> route.endpoint().startsWith(RouteCleanupRoutes.BASE_PATH))
