...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistrySupport {
    ...
    static OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
            response(
                    ...
                    List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                            .DigestSection> digestSections,
                    List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                            .ConsumerPacket> consumerPackets,
                    List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                            .ReplayInstruction> replayInstructions,
                    List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
        ...
        int lockedBoundaryCount = (int) boundaryLockCopy.stream()
                .filter(OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                        .BoundaryLock::locked)
                .count();
        int passedScorecardCount = countScorecard(scorecardCopy);

        List<String> checks = new ArrayList<>();
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-source-plan-"
                + SOURCE_PLAN);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-required-archive-"
                + REQUIRED_ARCHIVE_VERIFICATION_PLAN);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-operator-plan-"
                + OPERATOR_HANDOFF_PLAN);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-source-archive-version-"
                + sourceArchive.version());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-source-archive-status-"
                + sourceArchive.status());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-source-archive-count-"
                + sourceArchiveCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-section-count-"
                + digestSectionCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-passed-section-count-"
                + passedDigestSectionCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-consumer-packet-count-"
                + consumerPacketCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-ready-consumer-packet-count-"
                + readyConsumerPacketCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-replay-instruction-count-"
                + replayInstructionCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-read-only-replay-count-"
                + readOnlyReplayInstructionCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-boundary-lock-count-"
                + boundaryLockCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-locked-boundary-count-"
                + lockedBoundaryCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-scorecard-count-"
                + scorecardCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-passed-scorecard-count-"
                + passedScorecardCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-markdown-section-count-"
        ...
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-no-raw-endpoint-resolution");
        checks.add("minimal-read-only-gate-operator-ci-handoff-archive-digest-no-managed-audit-http");

        String status = "passed".equals(sourceArchive.status())
    ...
    private static int countReadyConsumerPackets(
            List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                    .ConsumerPacket> entries
    ) {
        return (int) entries.stream()
                .filter(OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                        .ConsumerPacket::ready)
                .count();
    }

    private static int countReadOnlyReplayInstructions(
            List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                    .ReplayInstruction> entries
    ) {
        return (int) entries.stream()
                .filter(OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                        .ReplayInstruction::readOnly)
                .count();
    }
