...
public class OpsShardReadinessRouteCleanupMaintenanceHandoffAcceptanceDigestService {
    ...
    public OpsShardReadinessRouteCleanupMaintenanceHandoffAcceptanceDigestResponse digest() {
        ...
        List<OpsShardReadinessRouteCleanupMaintenanceHandoffAcceptanceDigestResponse.AcceptanceSection>
                sections = List.of(
                section("owner-coverage", ownership.endpoint(), "operator-handoff-reviewer",
                        "distinct-owner-count-" + ownership.distinctOwnerCount(), ownership.status()),
                section("risk-closure", risks.endpoint(), "release-reviewer",
                        "mitigated-risk-count-" + risks.mitigatedRiskCount(), risks.status()),
                section("evidence-freshness", freshness.endpoint(), "archive-reviewer",
                        "stale-evidence-count-" + freshness.staleEvidenceCount(), freshness.status()),
                section("runtime-boundary", risks.endpoint(), "runtime-boundary-reviewer",
                        "executionAllowed-" + risks.executionAllowed(), risks.status()),
                section("handoff-readiness", ownership.endpoint(), "catalog-maintainer",
                        "owner-entry-count-" + ownership.ownerEntryCount(), ownership.status())
        );
        int accepted = (int) sections.stream().filter(section -> "passed".equals(section.status())).count();
    ...
    private OpsShardReadinessRouteCleanupMaintenanceHandoffAcceptanceDigestResponse.AcceptanceSection section(
            String name,
        ...
        return new OpsShardReadinessRouteCleanupMaintenanceHandoffAcceptanceDigestResponse.AcceptanceSection(
                name,
                sourceEndpoint,
                owner,
                evidence,
