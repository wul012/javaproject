...
final class OpsShardReadinessSignedApprovalDraftProfileSectionHandoffRenderer {
    ...
    static List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.RenderedHandoff> render(
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.SectionHandoff> handoffs
    ) {
        return handoffs.stream()
                .map(handoff -> new OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse
                        .RenderedHandoff(
                                handoff.order(),
                                handoff.sectionCode(),
                                "### " + handoff.heading(),
