...
final class OpsShardReadinessComparedEvidenceEvaluationPreflightSupport {
    ...
    static OpsShardReadinessComparedEvidenceEvaluationPreflightResponse response(
        ...
        int passedRuleCount = (int) ruleCopy.stream().filter(rule -> "passed".equals(rule.status())).count();
        ...
        checks.add("compared-evidence-evaluation-preflight-source-node-" + SOURCE_NODE_EVIDENCE_INTAKE_VERSION);
        checks.add("compared-evidence-evaluation-preflight-source-java-" + SOURCE_JAVA_REVIEW_HANDOFF_VERSION);
