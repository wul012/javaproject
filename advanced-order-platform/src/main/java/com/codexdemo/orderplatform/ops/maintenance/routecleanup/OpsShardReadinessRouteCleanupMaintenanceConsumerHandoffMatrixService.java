...
public class OpsShardReadinessRouteCleanupMaintenanceConsumerHandoffMatrixService {
  ...
  static final String ENDPOINT =
      RouteCleanupRoutes.BASE_PATH + RouteCleanupRoutes.MAINTENANCE_CONSUMER_HANDOFF_MATRIX;
  ...
  private String status(
      List<OpsShardReadinessRouteCleanupMaintenanceConsumerHandoffMatrixResponse.MatrixEntry>
          matrix,
      List<String> forbiddenOperations) {
    boolean passed =
        matrix.size() == OpsShardReadinessRouteCleanupMaintenanceUpkeepCatalog.items().size()
