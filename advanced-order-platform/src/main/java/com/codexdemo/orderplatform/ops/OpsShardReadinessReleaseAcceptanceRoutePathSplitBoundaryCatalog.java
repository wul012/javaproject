...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitBoundaryCatalog {
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse.BoundaryGuard> guards() {
        return List.of(
                ...
                guard("active-shard-router", "split records readiness route names but does not route traffic"),
                ...
                guard("deployment-rollback", "route-path split does not emit deployment or rollback actions"),
                guard("sibling-autostart", "Node v1846 parallel work does not authorize Java or mini-kv startup")
        );
    }
