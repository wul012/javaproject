...
final class OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckCatalog {
  ...
  private static final List<CheckpointSpec> CHECKPOINT_SPECS =
      List.of(
          spec(
              "material-source-package-submission-checkpoint",
              "material-source-package",
              "source package, material index",
              "source package, material index"),
          spec(
          ...
          spec(
              ...
              "field-table",
              "field table, field coverage",
              "field table, field coverage"),
          spec(
              "comparison-binding-submission-checkpoint",
              "comparison-binding",
              "comparison binding, lineage key",
              "comparison binding, lineage key"),
          spec(
              "signature-attestation-submission-checkpoint",
              "signature-attestation",
              "signature attestation, signer identity",
              "signature attestation, signer identity"),
          spec(
              "redaction-secret-boundary-submission-checkpoint",
              "redaction-secret-boundary",
              "redaction log, secret boundary",
              "redaction log, secret boundary"),
          spec(
              "runtime-import-freeze-submission-checkpoint",
              "runtime-import-freeze",
              "runtime freeze, import freeze",
              "runtime freeze, import freeze"),
          spec(
              "closeout-archive-submission-checkpoint",
              "closeout-archive",
              "archive index, absence attestation",
              "archive index, absence attestation"));

  private OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckCatalog() {}

  static List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.ModuleEntry>
      modules() {
    return List.of(
        module(
            209,
        ...
        module(
            213,
            "material-submission-precheck-route",
            "exposes the route and no-submission closeout evidence"));
  }

  static List<
          OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.SubmissionCheckpoint>
      checkpoints(OpsShardReadinessCandidateDocumentMaterialRequestResponse sourceRequest) {
    List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.SubmissionCheckpoint>
    ...
    for (int index = 0; index < CHECKPOINT_SPECS.size(); index++) {
      ...
      var sourceRequestCodes =
          sourceRequest.requestItems().subList(requestOffset, requestOffset + groupSize).stream()
              .map(OpsShardReadinessCandidateDocumentMaterialRequestResponse.RequestItem::code)
              .toList();
      var sourceAcceptanceCheckCodes =
          sourceRequest.acceptanceChecks().subList(checkOffset, checkOffset + groupSize).stream()
              .map(OpsShardReadinessCandidateDocumentMaterialRequestResponse.AcceptanceCheck::code)
              .toList();
      checkpoints.add(checkpoint(index + 1, spec, sourceRequestCodes, sourceAcceptanceCheckCodes));
  ...
  static List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Validator>
      validators(
          List<
    ...
    return checkpoints.stream()
        .map(
            checkpoint ->
                new OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Validator(
                    checkpoint.code() + "-validator",
                    checkpoint.code(),
                    "reject-material-submission-precheck-" + checkpoint.code(),
                    "Reject material submission until "
                        + checkpoint.submissionMaterialFields()
                        + " satisfy "
                        + checkpoint.precheck(),
                    "fail-closed",
                    "passed"))
  ...
  static List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Artifact>
      artifacts() {
    return List.of(
        artifact(
            "source-node-plan", "e/1162/source/node-v1456-material-submission-precheck-plan.md"),
        artifact("source-material-request", "e/1162/source/java-v1152-material-request.json"),
        artifact(
            "modules",
            "e/1162/modules/candidate-document-material-submission-precheck-modules.json"),
        artifact(
            "checkpoints",
            "e/1162/material/candidate-document-material-submission-checkpoints.json"),
        artifact(
            "validators", "e/1162/material/candidate-document-material-submission-validators.json"),
        artifact(
            "disabled-boundaries",
  ...
  static List<String> gates() {
    return IntStream.rangeClosed(
        ...
        .mapToObj(
            index -> "candidate-document-material-submission-precheck-no-material-gate-" + index)
        .toList();
  }

  private static OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.ModuleEntry
  ...
  private static OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse
          .SubmissionCheckpoint
      checkpoint(
          int order,
          CheckpointSpec spec,
  ...
  private static OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Artifact
      artifact(String code, String reference) {
    return new OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Artifact(
        code, reference, "material submission precheck evidence only", "passed");
  }
