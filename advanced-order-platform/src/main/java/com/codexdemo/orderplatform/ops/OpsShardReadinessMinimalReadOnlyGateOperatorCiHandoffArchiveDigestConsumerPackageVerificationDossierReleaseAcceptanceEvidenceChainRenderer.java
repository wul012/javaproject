...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierReleaseAcceptanceEvidenceChainRenderer {

    private OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierReleaseAcceptanceEvidenceChainRenderer() {
    }
    ...
    static OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierReleaseAcceptanceRegistryResponse
        ...
        return OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierReleaseAcceptanceRendererSupport
                .section("Evidence Chain", entries.stream()
                        .map(entry -> entry.order()
                                + ". " + entry.artifact()
                                + " | target=" + entry.releaseTarget()
                                + " | source=" + entry.sourceEvidence()
                                + " | status=" + entry.status())
                        .toList());
    }
}
