...
final class OpsShardReadinessComparedEvidenceEvaluationPreflightGuardCatalog {
  ...
  static List<OpsShardReadinessComparedEvidenceEvaluationPreflightResponse.EvaluationGuard>
      allGuards() {
    return OpsShardReadinessComparedEvidenceEvaluationPreflightRuleCatalog.allRules().stream()
        .map(
            rule ->
                OpsShardReadinessComparedEvidenceEvaluationPreflightSupport.guard(
                    rule.code() + "-guard",
                    rule.evaluationArea(),
  ...
  static List<OpsShardReadinessComparedEvidenceEvaluationPreflightResponse.EvaluationGuard>
      sourceArtifactGuards() {
    return allGuards().subList(0, 5);
  }

  static List<OpsShardReadinessComparedEvidenceEvaluationPreflightResponse.EvaluationGuard>
      identityDigestGuards() {
