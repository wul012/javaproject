...
final class OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCapturePreflightSupport {
  ...
  static OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCapturePreflightResponse
      response(
          String version,
          String endpoint,
          String profile,
    ...
    int passedInputCount =
        (int) inputCopy.stream().filter(input -> "passed".equals(input.status())).count();
    int passedAttestationCount =
        (int)
            attestationCopy.stream()
    ...
    checks.add(
        "signed-approval-capture-preflight-passed-attestation-count-" + passedAttestationCount);
    ...
    checks.add(
        "signed-approval-capture-preflight-source-review-" + SOURCE_APPROVAL_PACKET_REVIEW_VERSION);
    checks.add(
        "signed-approval-capture-preflight-source-java-preflight-"
  ...
  static OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCapturePreflightResponse
    ...
    return new OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCapturePreflightResponse
        .CapturePolicy(code, category, policy, enforcement);
  }
}
