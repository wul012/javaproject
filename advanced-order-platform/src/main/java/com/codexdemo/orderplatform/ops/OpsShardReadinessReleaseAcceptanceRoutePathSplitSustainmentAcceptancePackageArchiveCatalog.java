...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageArchiveCatalog {

    private OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageArchiveCatalog() {
    }
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageResponse
        ...
        return List.of(
                ...
                item("version-tags-v1580-v1604", "version-lineage", source.version(), "Java v1604"
                        .equals(source.version())),
                item("boundary-lock-matrix", "runtime-boundary-archive", "boundary-count="
                        + source.boundaryGuardCount(), allBoundariesLocked(source)),
                item("ci-gate-ledger", "ci-run-archive", "ci-gates=" + source.ciGateCount(),
                        allCiGatesRequired(source)),
                item("consumer-handoff-rules", "handoff-archive", "handoffs="
                        + source.consumerHandoffCount(), allConsumersReady(source))
        );
    }
