...
final class OpsShardReadinessCredentialResolverDisabledFakeHarnessEvidenceArchiveSupport {
  ...
  private static List<String> checks(
    ...
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-source-plan-" + SOURCE_PLAN);
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-owner-plan-" + NODE_OWNER_PLAN);
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-source-schema-"
            + rehearsal.verificationHint().responseSchemaVersion());
    sourceReceipts.stream()
        ...
        .ifPresent(
            source -> {
              result.add(
                  "credential-resolver-disabled-fake-harness-evidence-archive-source-receipt-"
                      + source.receiptVersion());
              result.add(
                  "credential-resolver-disabled-fake-harness-evidence-archive-source-digest-present-"
                      + source.receiptDigest().startsWith("sha256:"));
              result.add(
                  "credential-resolver-disabled-fake-harness-evidence-archive-consumes-"
                      + source.consumedNodeVersion());
              result.add(
                  "credential-resolver-disabled-fake-harness-evidence-archive-next-verification-"
                      + source.nextNodeVerificationVersion());
              result.add(
                  "credential-resolver-disabled-fake-harness-evidence-archive-fake-harness-deferred-until-"
                      + source.fakeHarnessDeferredUntil());
            });
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-java-requirements-"
            + javaRequirements.size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-mini-kv-requirements-"
            + miniKvRequirements.size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-fake-harness-boundaries-"
            + fakeHarnessBoundaries.size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-runtime-guards-"
            + runtimeGuards.size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-verification-gates-"
            + verificationGates.size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-handoff-notes-"
            + handoffNotes.size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-markdown-sections-"
            + markdownSections.size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-receipt-warnings-"
            + rehearsal
                .managedAuditSandboxEndpointCredentialResolverImplementationPlanEchoReceipt()
                .receiptWarnings()
                .size());
    result.add(
        "credential-resolver-disabled-fake-harness-evidence-archive-all-runtime-guards-passed-"
