...
final class OpsShardReadinessScreenshotExplanationArchiveRegistrySupport {
  ...
  static final String REGISTRY_STATE = "screenshot-explanation-archives-canonical-f-root-ready";
  ...
  static OpsShardReadinessScreenshotExplanationArchiveRegistryResponse response(
      ...
      String endpoint,
      String profile,
      List<OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.CurrentArchiveAssessment>
          currentArchiveAssessments,
      List<OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.ArchiveSegmentPlan>
          segmentPlans,
      List<OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.NamingRule> namingRules,
    ...
    boolean namingRulesRequired =
        namingRuleCopy.stream()
            .allMatch(
                OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.NamingRule::required);
    boolean verificationRequired =
        verificationStepCopy.stream()
            .allMatch(
                OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.VerificationStep
                    ::required);
    boolean hasNextRoot =
        currentArchiveCopy.stream().anyMatch(assessment -> NEXT_ROOT.equals(assessment.root()));
    boolean hasCurrentSegment =
        segmentPlanCopy.stream()
            .anyMatch(segment -> segment.path().contains("f/v1769-v1773") && segment.active());
    boolean statusPassed =
        currentArchiveCopy.size() == EXPECTED_CURRENT_ARCHIVE_ASSESSMENT_COUNT
    ...
    checks.add(
        "screenshot-explanation-archive-current-assessment-count-" + currentArchiveCopy.size());
    ...
    checks.add(
        "screenshot-explanation-archive-denied-boundary-rule-count-" + deniedBoundaryRuleCount);
    checks.add(
        "screenshot-explanation-archive-verification-step-count-" + verificationStepCopy.size());
