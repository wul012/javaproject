...
final class OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftTextPackageComparisonAcceptancePrecheckSupport {
    ...
    private OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftTextPackageComparisonAcceptancePrecheckSupport() {
    }
    ...
    response(
        ...
        int passedCheckpointCount = (int) checkpointCopy.stream()
                .filter(checkpoint -> "passed".equals(checkpoint.status())).count();
        int passedGuardCount = (int) guardCopy.stream().filter(guard -> "passed".equals(guard.status())).count();
        List<String> checks = new ArrayList<>();
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-checkpoint-count-"
                + checkpointCopy.size());
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-guard-count-"
                + guardCopy.size());
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-source-plan-"
                + SOURCE_PLAN);
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-source-node-"
                + SOURCE_NODE_COMPARISON_PREFLIGHT_VERSION);
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-source-java-"
                + SOURCE_JAVA_COMPARISON_PREFLIGHT_VERSION);
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-no-package-acceptance");
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-no-draft-text-parsing");
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-no-signature-parsing");
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-no-approval-grant");
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-no-runtime");
        checks.add("signed-approval-artifact-draft-text-package-comparison-acceptance-precheck-no-sibling-mutation");
    ...
    static OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftTextPackageComparisonAcceptancePrecheckResponse
        ...
        return new OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftTextPackageComparisonAcceptancePrecheckResponse
                .AcceptanceCheckpoint(code, sourceVersion, checkpoint, acceptanceQuestion, missingEvidenceGuard,
