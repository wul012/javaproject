...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentScorecardCatalog {
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentResponse.ScorecardEntry> scorecard(
        ...
        return List.of(
                entry("source-closeout", sourceSnapshots.stream()
                        .allMatch(snapshot -> "passed".equals(snapshot.status())),
                        "source closeout remains passed"),
                entry("ownership", ownershipRules.stream()
                ...
                entry("split-maintainability", true, "catalogs and renderers keep future work out of a giant file")
        );
    }

    private static OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentResponse.ScorecardEntry entry(
            String category,
            boolean passed,
            String detail
    ) {
