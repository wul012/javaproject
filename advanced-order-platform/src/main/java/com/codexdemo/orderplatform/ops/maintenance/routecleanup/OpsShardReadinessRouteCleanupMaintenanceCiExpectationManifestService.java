...
public class OpsShardReadinessRouteCleanupMaintenanceCiExpectationManifestService {
  ...
  static final String ENDPOINT =
      RouteCleanupRoutes.BASE_PATH + RouteCleanupRoutes.MAINTENANCE_CI_EXPECTATION_MANIFEST;
  ...
  public OpsShardReadinessRouteCleanupMaintenanceCiExpectationManifestResponse manifest() {
    List<OpsShardReadinessRouteCleanupMaintenanceCiExpectationManifestResponse.CiExpectation>
        expectations =
  ...
  private OpsShardReadinessRouteCleanupMaintenanceCiExpectationManifestResponse.CiExpectation
      expectation(OpsShardReadinessRouteCleanupMaintenanceUpkeepCatalog.Item item) {
  ...
  private String focusedTestClass(String itemName) {
    return switch (itemName) {
      case "segment-catalog" ->
          "OpsShardReadinessRouteCleanupMaintenanceSegmentCatalogServiceTests";
      case "continuity" -> "OpsShardReadinessRouteCleanupMaintenanceContinuityServiceTests";
      case "latest-sibling-report" ->
          "OpsShardReadinessRouteCleanupMaintenanceLatestSiblingServiceTests";
      case "handoff-pair-audit" ->
          "OpsShardReadinessRouteCleanupMaintenanceHandoffPairAuditServiceTests";
      case "boundary-drift" -> "OpsShardReadinessRouteCleanupMaintenanceBoundaryDriftServiceTests";
      case "source-plan-alignment" ->
          "OpsShardReadinessRouteCleanupMaintenanceSourcePlanAlignmentServiceTests";
      case "test-budget-plan" ->
          "OpsShardReadinessRouteCleanupMaintenanceTestBudgetPlanServiceTests";
      case "archive-manifest" ->
          "OpsShardReadinessRouteCleanupMaintenanceArchiveManifestServiceTests";
  ...
  private String status(
    ...
    boolean passed =
        expectations.size() == 9
            && expectations.stream()
                .allMatch(expectation -> expectation.focusedTestClass().endsWith("Tests"))
            && expectations.stream()
                .allMatch(expectation -> expectation.fullRegressionCommand().equals("mvn -q test"))
