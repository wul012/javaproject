  ...
  static OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse response(
      ...
      List<
              OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse
                  .SourceLineage>
          sourceLineage,
      List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ModuleEntry>
          modules,
      List<
              OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse
                  .ArchiveHandle>
          archiveHandles,
      List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.PolicyLock>
          policyLocks,
      List<
              OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse
                  .ArtifactReference>
          artifactReferences,
      List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ConsumerRule>
          consumerRules,
      List<String> gates,
    ...
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-plan-" + SOURCE_PLAN);
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-node-"
            + SOURCE_NODE_MATERIAL_SUBMISSION_PRECHECK_VERSION);
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-java-precheck-"
            + sourcePrecheck.version());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-route-"
            + sourcePrecheck.endpoint());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-lineage-count-"
            + lineageCopy.size());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-module-count-"
            + moduleCopy.size());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-archive-handle-count-"
            + archiveCopy.size());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-policy-lock-count-"
            + policyCopy.size());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-artifact-reference-count-"
            + artifactCopy.size());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-consumer-rule-count-"
            + consumerCopy.size());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-checkpoint-count-"
            + sourcePrecheck.checkpointCount());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-validator-count-"
            + sourcePrecheck.validatorCount());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-artifact-count-"
            + sourcePrecheck.artifactCount());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-source-gate-count-"
            + sourcePrecheck.gateCount());
    checks.add(
        "candidate-document-material-submission-precheck-handoff-gate-count-" + gateCopy.size());
    ...
    checks.add(
        "candidate-document-material-submission-precheck-handoff-signature-capture-disabled");
