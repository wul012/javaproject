...
public class OpsShardReadinessRouteCleanupMaintenanceRemediationQueueService {
    ...
    public OpsShardReadinessRouteCleanupMaintenanceRemediationQueueResponse queue() {
        ...
        List<OpsShardReadinessRouteCleanupMaintenanceRemediationQueueResponse.QueueItem> items = List.of(
                item("fail-closed-policy-drift", "policy-zero-violations", policy.endpoint(),
                        "inspect-policy-report-before-any-code-change", policy.status()),
                item("readiness-gate-blocked", "blocked-gate-check-count-" + gate.blockedCheckCount(),
                        gate.endpoint(), "review-gate-check-reason", gate.status()),
                item("execution-boundary-drift", "executionAllowed-" + gate.executionAllowed(),
                        gate.endpoint(), "keep-execution-disabled", gate.status()),
                item("upstream-startup-drift", "starts-upstream-false", gate.endpoint(),
                        "do-not-start-java-or-mini-kv", gate.status())
        );
