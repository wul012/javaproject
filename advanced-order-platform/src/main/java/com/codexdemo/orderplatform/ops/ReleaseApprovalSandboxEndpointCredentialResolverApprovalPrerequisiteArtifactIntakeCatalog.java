...
final class ReleaseApprovalSandboxEndpointCredentialResolverApprovalPrerequisiteArtifactIntakeCatalog {

    private static final List<RequiredFieldTemplate> REQUIRED_FIELD_TEMPLATES = List.of(
            requiredField("artifact_id", "Artifact id", "operator", "stable non-secret id",
                    "Trace this intake artifact across Node, Java, and mini-kv evidence."),
            requiredField("source_node_verification", "Source Node verification", "node-v305",
                    "Node v305 digest + route reference", "Bind the artifact to the v305 upstream echo verification."),
            requiredField("operator_approval_reference", "Operator approval reference", "operator",
                    "non-secret approval ticket or review id",
                    "Prove a human approval review exists before any runtime shell step."),
            requiredField("credential_handle_review_status", "Credential handle review status", "audit-process",
                    "credential handle + status, no value", "Show that only a credential handle was reviewed."),
            requiredField("endpoint_handle_allowlist_review_status", "Endpoint handle allowlist review status",
                    "audit-process", "endpoint handle + allowlist status, no raw URL",
                    "Show that only an endpoint handle was reviewed."),
            requiredField("no_network_safety_test_reference", "No-network safety test reference", "node-v305",
                    "test or report id", "Prove the intake remains offline and does not send HTTP/TCP."),
            requiredField("manual_abort_semantics_reference", "Manual abort semantics reference", "operator",
                    "runbook or review id", "Document how an operator stops before any runtime shell invocation."),
            requiredField("rollback_semantics_reference", "Rollback semantics reference", "operator",
                    "runbook or review id", "Document rollback expectations without executing deployment or rollback."),
            requiredField("java_echo_required_version", "Java echo required version", "java-v142",
                    "Java v142 receipt reference", "Tell Java exactly which read-only artifact contract to echo."),
            requiredField("mini_kv_receipt_required_version", "mini-kv receipt required version", "mini-kv-v135",
                    "mini-kv v135 receipt reference", "Tell mini-kv exactly which non-participation receipt to emit."),
            requiredField("created_by_operator_identity", "Created by operator identity", "operator",
                    "operator id or identity handle", "Bind the artifact to a non-secret operator identity."),
            requiredField("audit_correlation_id", "Audit correlation id", "audit-process",
                    "stable correlation id", "Link all later evidence without embedding secret material.")
    );

    private static final List<ProhibitedFieldTemplate> PROHIBITED_FIELD_TEMPLATES = List.of(
            ...
            prohibitedField("schema_migration_sql",
                    ...
                    "WRITE_OR_SCHEMA_MUTATION_REQUESTED"),
            prohibitedField("mini_kv_write_command",
                    "mini-kv must remain non-authoritative and read-only.",
                    "MINI_KV_WRITE_OR_AUTHORITY_REQUESTED")
    );

    private static final List<RejectionReasonTemplate> REJECTION_REASON_TEMPLATES = List.of(
            rejectionReason("MISSING_OPERATOR_APPROVAL_REFERENCE", "operator-approval",
                    "Reject artifacts without a non-secret operator approval reference."),
            rejectionReason("CREDENTIAL_VALUE_PRESENT", "credential-boundary",
                    "Reject artifacts that include credential values instead of handles."),
            rejectionReason("RAW_ENDPOINT_URL_PRESENT", "endpoint-boundary",
                    "Reject artifacts that include raw endpoint URLs instead of endpoint handles."),
            rejectionReason("NO_NETWORK_SAFETY_TEST_MISSING", "runtime-boundary",
                    "Reject artifacts that do not cite a no-network safety check."),
            rejectionReason("ABORT_ROLLBACK_SEMANTICS_MISSING", "runtime-boundary",
                    "Reject artifacts missing abort or rollback semantics."),
            rejectionReason("JAVA_OR_MINIKV_ECHO_MISSING", "upstream-echo",
                    "Reject completion claims until Java v142 and mini-kv v135 echo this contract."),
            rejectionReason("RUNTIME_SHELL_IMPLEMENTATION_REQUESTED", "runtime-boundary",
                    "Reject requests to implement, enable, or invoke a runtime shell."),
            rejectionReason("EXTERNAL_REQUEST_REQUESTED", "runtime-boundary",
                    "Reject requests to send HTTP/TCP or instantiate providers/clients."),
            rejectionReason("WRITE_OR_SCHEMA_MUTATION_REQUESTED", "write-boundary",
                    "Reject ledger writes, SQL migrations, deployments, rollbacks, and mini-kv write/admin commands.")
    );

    private static final List<NoGoBoundaryTemplate> NO_GO_BOUNDARY_TEMPLATES = List.of(
            ...
            noGoBoundary("provider_or_client_instantiated",
            ...
            noGoBoundary("automatic_upstream_start",
    ...
    private static final List<String> NODE_WARNING_CODES = List.of(
            "ARTIFACT_INTAKE_PLAN_DOES_NOT_AUTHORIZE_RUNTIME"
    );
    ...
    private ReleaseApprovalSandboxEndpointCredentialResolverApprovalPrerequisiteArtifactIntakeCatalog() {
    }

    static List<RehearsalApprovalPrerequisiteArtifactRequiredField> requiredFields() {
        return REQUIRED_FIELD_TEMPLATES.stream()
                .map(template -> new RehearsalApprovalPrerequisiteArtifactRequiredField(
                        template.id(),
                        template.label(),
                        true,
                        template.source(),
    ...
    static List<RehearsalApprovalPrerequisiteArtifactProhibitedField> prohibitedFields() {
        return PROHIBITED_FIELD_TEMPLATES.stream()
                .map(template -> new RehearsalApprovalPrerequisiteArtifactProhibitedField(
                        template.id(),
                        template.reason(),
    ...
    static List<RehearsalApprovalPrerequisiteArtifactRejectionReason> rejectionReasons() {
        return REJECTION_REASON_TEMPLATES.stream()
                .map(template -> new RehearsalApprovalPrerequisiteArtifactRejectionReason(
                        template.code(),
                        template.source(),
                        template.message()
                ))
    ...
    static List<RehearsalApprovalPrerequisiteArtifactNoGoBoundary> noGoBoundaries() {
        return NO_GO_BOUNDARY_TEMPLATES.stream()
                .map(template -> new RehearsalApprovalPrerequisiteArtifactNoGoBoundary(
                        template.id(),
                        false,
                        template.message()
                ))
    ...
    private static RejectionReasonTemplate rejectionReason(String code, String source, String message) {
    ...
    private record RequiredFieldTemplate(String id, String label, String source, String acceptedShape, String purpose) {
    }
