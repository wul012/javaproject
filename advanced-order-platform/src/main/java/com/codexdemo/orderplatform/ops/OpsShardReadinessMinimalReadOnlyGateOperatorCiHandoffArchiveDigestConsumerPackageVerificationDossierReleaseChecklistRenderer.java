...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierReleaseChecklistRenderer {

    private OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierReleaseChecklistRenderer() {
    }
    ...
    static OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierRegistryResponse
        ...
        return OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierRendererSupport
                .section("Release Checklist", entries.stream()
                        .map(entry -> entry.order()
                                + ". " + entry.item()
                                + " | owner=" + entry.owner()
                                + " | evidence=" + entry.releaseEvidence()
                                + " | status=" + entry.status())
                        .toList());
    }
}
