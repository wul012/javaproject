...
final
class OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftAuthoringReadinessSupport {
  ...
  private
  OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftAuthoringReadinessSupport() {}
  ...
  OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftAuthoringReadinessResponse
    ...
    int passedRequirementCount =
        (int)
            requirementCopy.stream()
                .filter(requirement -> "passed".equals(requirement.status()))
                .count();
    int passedBlockerCount =
        (int) blockerCopy.stream().filter(blocker -> "passed".equals(blocker.status())).count();
    List<String> checks = new ArrayList<>();
    checks.add(
        "signed-approval-artifact-draft-authoring-readiness-requirement-count-"
            + requirementCopy.size());
    checks.add(
        "signed-approval-artifact-draft-authoring-readiness-passed-requirement-count-"
            + passedRequirementCount);
    checks.add(
        "signed-approval-artifact-draft-authoring-readiness-blocker-count-" + blockerCopy.size());
    checks.add(
        "signed-approval-artifact-draft-authoring-readiness-passed-blocker-count-"
            + passedBlockerCount);
    ...
    checks.add(
        "signed-approval-artifact-draft-authoring-readiness-source-node-review-package-preflight-"
            + SOURCE_NODE_REVIEW_PACKAGE_PREFLIGHT_VERSION);
    checks.add(
        "signed-approval-artifact-draft-authoring-readiness-source-java-review-package-preflight-"
  ...
  OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftAuthoringReadinessResponse
    ...
    return new OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftAuthoringReadinessResponse
        .AuthoringBlocker(code, category, blocker, rejectionCode, enforcement, "passed");
  }
