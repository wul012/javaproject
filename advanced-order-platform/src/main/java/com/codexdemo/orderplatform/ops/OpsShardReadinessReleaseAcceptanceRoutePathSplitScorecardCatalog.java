...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitScorecardCatalog {
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse.ScorecardEntry> scorecard(
        ...
        List<OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse.ScorecardEntry> entries =
                new ArrayList<>();
        entries.add(entry("source-handoff", sourceSnapshots.size() == 1,
                "Java v1547 handoff remains the source evidence"));
        entries.add(entry("stable-barrel", routePaths.stream().allMatch(
                OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse.RoutePathEntry::legacyCompatible),
                "OpsShardReadinessRoutePaths keeps every migrated constant"));
        entries.add(entry("narrow-module", routePaths.size() == 11,
                "release-acceptance route ownership has a dedicated module"));
        entries.add(entry("compatibility", compatibilityChecks.stream().allMatch(
                OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse.CompatibilityCheck::matched),
                "stable and split route values match one-for-one"));
        entries.add(entry("consumer-migration", consumerHandoffs.size() == 5,
                "new consumers know whether to use the narrow module or legacy barrel"));
        entries.add(entry("boundary-lock", boundaryGuards.stream().allMatch(
        ...
        entries.add(entry("node-v1846-parallelism", true,
                "Java proceeds independently while Node owns its type-barrel split"));
        entries.add(entry("maintainability", true,
