...
public class OpsShardReadinessRouteCleanupMaintenanceHandoffPairAuditService {
  ...
  public static final String ENDPOINT =
      RouteCleanupRoutes.BASE_PATH + RouteCleanupRoutes.MAINTENANCE_HANDOFF_PAIR_AUDIT;
  static final String PROFILE =
      "java-shard-readiness-route-cleanup-maintenance-handoff-pair-audit.v1";

  private static final Set<String> ALLOWED_ROUTE_ONLY_PHASES =
      Set.of("handoff-suite-closeout-route", "handoff-suite-completion-certificate-route");
  ...
  public OpsShardReadinessRouteCleanupMaintenanceHandoffPairAuditResponse audit() {
    ...
    Set<String> routeKeys =
        phases.stream()
            ...
            .collect(java.util.stream.Collectors.toSet());
    List<String> routeOnly =
        routeKeys.stream()
            .filter(key -> !serviceKeys.contains(key))
            .map(key -> key + "-route")
            .sorted()
            .toList();
    List<String> unpairedServices =
        serviceKeys.stream()
            .filter(key -> !routeKeys.contains(key))
            .map(key -> key + "-service")
            .sorted()
            .toList();
    int pairedRouteCount = (int) routeKeys.stream().filter(serviceKeys::contains).count();
    List<String> checks =
        List.of(
