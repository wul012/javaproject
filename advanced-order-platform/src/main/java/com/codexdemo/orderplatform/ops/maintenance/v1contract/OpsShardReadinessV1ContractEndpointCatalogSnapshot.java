...
final class OpsShardReadinessV1ContractEndpointCatalogSnapshot {
  ...
  static OpsShardReadinessV1ContractEndpointCatalogResponse v208Catalog() {
    List<OpsShardReadinessV1ContractEndpointCatalogResponse.EndpointEntry> entries =
        v208EndpointEntries();
    return new OpsShardReadinessV1ContractEndpointCatalogResponse(
        ...
        entries.size(),
        entries,
        entries.stream().map(entry -> "GET " + entry.liveEndpoint()).toList(),
        entries.stream().map(entry -> "GET " + entry.fixtureEndpoint()).toList(),
        entries.stream()
  ...
  static List<OpsShardReadinessV1ContractEndpointCatalogResponse.EndpointEntry>
      v208EndpointEntries() {
    return List.of(
        endpointEntry(
            "alignment",
  ...
  private static OpsShardReadinessV1ContractEndpointCatalogResponse.EndpointEntry endpointEntry(
    ...
    return new OpsShardReadinessV1ContractEndpointCatalogResponse.EndpointEntry(
        name, liveEndpoint, fixtureEndpoint, evidencePath, receiptId);
  }
}
