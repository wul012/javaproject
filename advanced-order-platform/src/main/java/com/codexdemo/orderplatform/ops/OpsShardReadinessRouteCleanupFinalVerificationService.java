...
public class OpsShardReadinessRouteCleanupFinalVerificationService {
    ...
    public OpsShardReadinessRouteCleanupFinalVerificationResponse verification() {
        ...
        List<OpsShardReadinessRouteCleanupFinalVerificationResponse.Verification> verifications = List.of(
                verification("transition-brief", brief.transitionBriefEndpoint() + ":" + brief.status()),
                verification("reviewer-packet", packet.reviewerPacketEndpoint() + ":" + packet.status()),
                verification("final-digest", digest.digestEndpoint() + ":" + digest.digestValue()),
                verification("versions-continuous",
                        String.valueOf(OpsShardReadinessRouteCleanupEvidenceAnalyzer.versionsAreContinuous())),
                verification("read-only-boundary",
                        OpsShardReadinessRouteCleanupEvidenceAnalyzer.boundaryStatus())
        );
        boolean passed = brief.status().equals("passed")
