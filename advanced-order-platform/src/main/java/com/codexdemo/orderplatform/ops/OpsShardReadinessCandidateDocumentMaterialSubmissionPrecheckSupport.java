...
final class OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckSupport {
    ...
    static OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse response(
            ...
            List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.ModuleEntry> modules,
            List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.SubmissionCheckpoint> checkpoints,
            List<OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Validator> validators,
        ...
        int passedCheckpointCount = (int) checkpointCopy.stream()
                ...
                .count();
        int passedValidatorCount = (int) validatorCopy.stream()
                .filter(validator -> "passed".equals(validator.status()))
                .count();
        List<String> checks = new ArrayList<>();
        checks.add("candidate-document-material-submission-precheck-source-plan-" + SOURCE_PLAN);
        checks.add("candidate-document-material-submission-precheck-source-node-"
                + SOURCE_NODE_MATERIAL_REQUEST_VERSION);
        checks.add("candidate-document-material-submission-precheck-source-java-material-request-"
                + sourceRequest.version());
        checks.add("candidate-document-material-submission-precheck-source-route-"
                + sourceRequest.endpoint());
        checks.add("candidate-document-material-submission-precheck-module-count-" + moduleCopy.size());
        checks.add("candidate-document-material-submission-precheck-checkpoint-count-"
                + checkpointCopy.size());
        checks.add("candidate-document-material-submission-precheck-validator-count-"
                + validatorCopy.size());
        checks.add("candidate-document-material-submission-precheck-source-request-item-count-"
                + sourceRequest.requestItemCount());
        checks.add("candidate-document-material-submission-precheck-source-acceptance-check-count-"
                + sourceRequest.acceptanceCheckCount());
        checks.add("candidate-document-material-submission-precheck-required-field-count-"
                + EXPECTED_REQUIRED_MATERIAL_FIELD_COUNT);
        checks.add("candidate-document-material-submission-precheck-submission-field-count-"
                + EXPECTED_SUBMISSION_MATERIAL_FIELD_COUNT);
        checks.add("candidate-document-material-submission-precheck-artifact-count-"
                + artifactCopy.size());
