...
final
class OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierPreconditionEvidenceCatalog {
  ...
  static List<
    ...
    return rehearsal
        ...
        .map(
            source ->
                new OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse
                    .PreconditionEvidence(id(source), source, true, true))
        .toList();
  }

  private static String id(String evidence) {
    return evidence.substring(0, evidence.indexOf(':')).replace(' ', '-').toLowerCase();
  }
}
