...
public class OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistService {
  ...
  static final String ENDPOINT =
      RouteCleanupRoutes.BASE_PATH + RouteCleanupRoutes.MAINTENANCE_RELEASE_CHECKLIST;
  ...
  public OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistResponse checklist() {
    ...
    List<OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistResponse.ChecklistItem> items =
        List.of(
            item(
                "catalog-baseline",
                catalog.endpoint(),
                "catalog-maintainer",
                "upkeep-items-" + catalog.itemCount(),
                catalog.status()),
            item(
                "readiness-gate",
                gate.endpoint(),
                "release-reviewer",
                "accepted-checks-" + gate.acceptedCheckCount(),
                gate.status()),
            item(
                "closeout",
                closeout.endpoint(),
                "operator-handoff-reviewer",
                "checked-reports-" + closeout.checkedReportCount(),
                closeout.status()),
            item(
                "source-plan",
                closeout.endpoint(),
  ...
  private OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistResponse.ChecklistItem item(
    ...
    return new OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistResponse.ChecklistItem(
        name, sourceEndpoint, owner, evidence, status);
  }
}
