...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierHandoffReceiptCatalog {

    private OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierHandoffReceiptCatalog() {
    }
    ...
    static List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierRegistryResponse
        ...
        return List.of(
                ...
                receipt("node-v368-archive-verifier", source.sourceDigestVersion(), "archive-verification-input", source),
                receipt("node-v369-operator-ci", source.profile(), "operator-ci-handoff-input", source),
                receipt("java-read-only-boundary-owner", source.endpoint(), "read-only-boundary-continuity", source)
        );
    }
    ...
    private static OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierRegistryResponse
        ...
        boolean ready = "passed".equals(source.status()) && sourceEvidence != null && !sourceEvidence.isBlank();
