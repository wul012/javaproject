...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageAcceptanceCatalog {

    private OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageAcceptanceCatalog() {
    }

    static List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageRegistryResponse
            .AcceptanceCriterion> criteria(
                    OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse source
            ) {
        return List.of(
                criterion("source-digest-passed", "status=" + source.status(),
                        "passed".equals(source.status())),
                criterion("digest-sections-passed",
                        "digest-sections=" + source.passedDigestSectionCount() + "/"
                                + source.digestSectionCount(),
                        source.passedDigestSectionCount() == source.digestSectionCount()),
                criterion("consumer-packets-ready",
                        "consumer-packets=" + source.readyConsumerPacketCount() + "/"
                                + source.consumerPacketCount(),
                        source.readyConsumerPacketCount() == source.consumerPacketCount()),
                criterion("replay-instructions-read-only",
                        "replay=" + source.readOnlyReplayInstructionCount() + "/"
                                + source.replayInstructionCount(),
                        source.readOnlyReplayInstructionCount() == source.replayInstructionCount()),
                criterion("boundaries-locked",
