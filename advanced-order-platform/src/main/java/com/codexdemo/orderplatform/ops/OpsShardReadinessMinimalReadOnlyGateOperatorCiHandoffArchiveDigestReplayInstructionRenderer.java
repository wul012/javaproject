...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestReplayInstructionRenderer {

    private OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestReplayInstructionRenderer() {
    }
    ...
    static OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
            .MarkdownSection render(
                    List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestRegistryResponse
                            .ReplayInstruction> instructions
            ) {
        List<String> lines = new ArrayList<>();
        lines.add("replay-instruction-count=" + instructions.size());
        instructions.forEach(instruction -> lines.add(instruction.order()
                + ". "
                + instruction.batch()
                + " | "
