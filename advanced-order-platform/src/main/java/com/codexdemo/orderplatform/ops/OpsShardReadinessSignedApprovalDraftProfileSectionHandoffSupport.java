...
final class OpsShardReadinessSignedApprovalDraftProfileSectionHandoffSupport {
    ...
    static OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse response(
            ...
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.SectionHandoff> sectionHandoffs,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.RouteContract> routeContracts,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.BoundaryDecision> boundaryDecisions,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.RenderedHandoff> renderedHandoffs,
            List<String> gates,
        ...
        int transferredLockedRouteFieldCount = contractCopy.stream()
                .mapToInt(OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse
                        .RouteContract::lockedFieldCount)
                .sum();
        List<String> checks = new ArrayList<>();
        checks.add("signed-approval-draft-profile-section-handoff-source-plan-" + SOURCE_PLAN);
        checks.add("signed-approval-draft-profile-section-handoff-source-registry-version-"
                + registry.version());
        ...
        checks.add("signed-approval-draft-profile-section-handoff-route-contract-count-"
                + contractCopy.size());
        checks.add("signed-approval-draft-profile-section-handoff-boundary-decision-count-"
                + decisionCopy.size());
        checks.add("signed-approval-draft-profile-section-handoff-rendered-count-" + renderedCopy.size());
        checks.add("signed-approval-draft-profile-section-handoff-gate-count-" + gateCopy.size());
        checks.add("signed-approval-draft-profile-section-handoff-transferred-route-lock-count-"
                + contractCopy.size());
        checks.add("signed-approval-draft-profile-section-handoff-transferred-locked-field-count-"
        ...
        return new OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse(
                ...
                gateCopy,
                List.copyOf(checks),
                isComplete(moduleCopy, sourceCopy, handoffCopy, contractCopy, decisionCopy,
                        renderedCopy, gateCopy, registry, transferredLockedRouteFieldCount)
                        ? "passed"
                        : "blocked"
        );
    }
    ...
    private static boolean isComplete(
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.ModuleEntry> modules,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.HandoffSource> sources,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.SectionHandoff> handoffs,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.RouteContract> contracts,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.BoundaryDecision> decisions,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse.RenderedHandoff> rendered,
            List<String> gates,
            OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse registry,
