...
final class ReleaseApprovalSandboxEndpointCredentialResolverAbortRollbackSemanticsContractCatalog {
    ...
    private static final List<RequiredFieldTemplate> REQUIRED_FIELDS = List.of(
            requiredField("manual_abort_marker", "Manual abort marker",
                    "stable non-secret manual abort marker",
                    "Bind the future path to a human-visible abort marker."),
            requiredField("rollback_runbook_reference", "Rollback runbook reference",
    ...
    private static final List<ProhibitedFieldTemplate> PROHIBITED_FIELDS = List.of(
            ...
            prohibitedField("java_sql_execution",
    ...
    private static final List<RejectionTemplate> REJECTION_REASONS = List.of(
            rejection("MANUAL_ABORT_MARKER_MISSING", "abort-rollback-semantics-contract",
                    "The manual abort marker is missing."),
            rejection("ROLLBACK_RUNBOOK_REFERENCE_MISSING", "abort-rollback-semantics-contract",
            ...
            rejection("RUNTIME_SHELL_COMMAND_PRESENT", "runtime-shell-boundary",
                    "Runtime shell commands and shell script bodies are prohibited."),
            rejection("NETWORK_OR_PROVIDER_ACTION_PRESENT", "network-boundary",
                    "Network execution, provider/client config, HTTP requests, and TCP attempts are prohibited."),
            rejection("WRITE_OR_ROLLBACK_ACTION_PRESENT", "write-boundary",
    ...
    private static final List<EchoRequestTemplate> UPSTREAM_ECHO_REQUESTS = List.of(
            ...
            echoRequest("mini-kv", "mini-kv v142",
    ...
    static List<RehearsalAbortRollbackSemanticsRequiredField> requiredFields() {
        return REQUIRED_FIELDS.stream()
                .map(template -> new RehearsalAbortRollbackSemanticsRequiredField(
    ...
    static List<RehearsalAbortRollbackSemanticsProhibitedField> prohibitedFields() {
        return PROHIBITED_FIELDS.stream()
                .map(template -> new RehearsalAbortRollbackSemanticsProhibitedField(
    ...
    static List<RehearsalAbortRollbackSemanticsRejectionReason> rejectionReasons() {
        return REJECTION_REASONS.stream()
                .map(template -> new RehearsalAbortRollbackSemanticsRejectionReason(
    ...
    static List<RehearsalAbortRollbackSemanticsNoGoBoundary> noGoBoundaries() {
        return NO_GO_BOUNDARIES.stream()
                .map(template -> new RehearsalAbortRollbackSemanticsNoGoBoundary(
    ...
    static List<RehearsalAbortRollbackSemanticsUpstreamEchoRequest> upstreamEchoRequests() {
        return UPSTREAM_ECHO_REQUESTS.stream()
                .map(template -> new RehearsalAbortRollbackSemanticsUpstreamEchoRequest(
