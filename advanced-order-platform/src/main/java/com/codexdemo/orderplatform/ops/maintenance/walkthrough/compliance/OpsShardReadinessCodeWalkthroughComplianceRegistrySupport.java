...
final class OpsShardReadinessCodeWalkthroughComplianceRegistrySupport {
  ...
  static final String ARCHIVE_DIRECTORY = "代码讲解记录_生产雏形阶段4/v1728-v1747";
  ...
  static OpsShardReadinessCodeWalkthroughComplianceRegistryResponse response(
    ...
    boolean versionsStandard =
        versionCopy.stream().allMatch(entry -> "standard".equals(entry.status()));
    boolean statusPassed =
        versionCopy.size() == EXPECTED_VERSION_COUNT
    ...
    checks.add(
        "code-walkthrough-compliance-documentation-rule-count-" + documentationRuleCopy.size());
    checks.add("code-walkthrough-compliance-boundary-rule-count-" + boundaryRuleCopy.size());
    checks.add("code-walkthrough-compliance-denied-boundary-rule-count-" + deniedBoundaryRuleCount);
    checks.add("code-walkthrough-compliance-test-coverage-count-" + testCoverageCopy.size());
    checks.add("code-walkthrough-compliance-markdown-section-count-" + markdownSectionCopy.size());
    checks.add("code-walkthrough-compliance-future-cutoff-v" + LEGACY_MARKER_CUTOFF_VERSION);
    checks.add(
        "code-walkthrough-compliance-legacy-marked-count-" + LEGACY_MARKED_WALKTHROUGH_COUNT);
