...
final class OpsShardReadinessCandidateDocumentRequestPackageSupport {
  ...
  static final String REQUEST_PACKAGE_STATE =
  ...
  static OpsShardReadinessCandidateDocumentRequestPackageResponse response(
      ...
      List<OpsShardReadinessCandidateDocumentRequestPackageResponse.AcceptanceCheck>
          acceptanceChecks,
      List<String> gates,
    ...
    int passedCheckCount =
        (int) checkCopy.stream().filter(check -> "passed".equals(check.status())).count();
    ...
    checks.add(
        "candidate-document-request-package-requested-field-count-"
    ...
    checks.add(
        "candidate-document-request-package-source-node-" + SOURCE_NODE_CANDIDATE_INTAKE_VERSION);
    checks.add(
        "candidate-document-request-package-source-java-" + SOURCE_JAVA_CANDIDATE_INTAKE_VERSION);
    ...
    return new OpsShardReadinessCandidateDocumentRequestPackageResponse(
        ...
        List.copyOf(checks),
        itemCopy.size() == 15
                && passedItemCount == itemCopy.size()
                && checkCopy.size() == 15
                && passedCheckCount == checkCopy.size()
  ...
  static OpsShardReadinessCandidateDocumentRequestPackageResponse.RequestItem item(
    ...
    return new OpsShardReadinessCandidateDocumentRequestPackageResponse.RequestItem(
        code, sourceIntakeSlot, requestedFields, instruction, owner, sourceEndpoint, "passed");
  }

  static OpsShardReadinessCandidateDocumentRequestPackageResponse.AcceptanceCheck check(
      String code, String category, String check, String rejectionCode) {
    return new OpsShardReadinessCandidateDocumentRequestPackageResponse.AcceptanceCheck(
        code, category, check, rejectionCode, "fail-closed", "passed");
  }
}
