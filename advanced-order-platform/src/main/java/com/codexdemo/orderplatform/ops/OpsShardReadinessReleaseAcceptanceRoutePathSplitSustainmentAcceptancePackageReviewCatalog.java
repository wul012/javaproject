...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageReviewCatalog {

    private OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageReviewCatalog() {
    }
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageResponse
        ...
        return List.of(
                ...
                item("route-owner", "route-delegate", "stable delegate remains enforced",
                        ownershipHeld(source, "stable-route-delegate")),
                item("test-owner", "coverage", "catalog, renderer, controller, and immutability tests exist",
                        ownershipHeld(source, "test-ownership")),
                item("ci-owner", "ci-gates", "five CI gates are required", source.ciGateCount() == 5),
                item("archive-owner", "archive-items", "acceptance package is ready for retention", passed)
        );
    }
