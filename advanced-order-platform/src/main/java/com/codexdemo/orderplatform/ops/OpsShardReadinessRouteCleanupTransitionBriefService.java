...
public class OpsShardReadinessRouteCleanupTransitionBriefService {
    ...
    public OpsShardReadinessRouteCleanupTransitionBriefResponse brief() {
        ...
        OpsShardReadinessRouteCleanupOperationalSnapshotResponse snapshot = operationalSnapshotService.snapshot();
        OpsShardReadinessRouteCleanupPolicyGuardResponse guard = policyGuardService.guard();
        List<OpsShardReadinessRouteCleanupTransitionBriefResponse.ReadinessSignal> signals = List.of(
                ...
                signal("execution-boundary", String.valueOf(!packet.executionAllowed()
                        && !snapshot.executionAllowed()
                        && !guard.executionAllowed()))
        );
        boolean passed = packet.status().equals("passed")
