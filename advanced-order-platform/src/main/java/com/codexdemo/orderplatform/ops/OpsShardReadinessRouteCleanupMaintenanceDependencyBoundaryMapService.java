...
public class OpsShardReadinessRouteCleanupMaintenanceDependencyBoundaryMapService {
    ...
    public OpsShardReadinessRouteCleanupMaintenanceDependencyBoundaryMapResponse map() {
        List<OpsShardReadinessRouteCleanupMaintenanceDependencyBoundaryMapResponse.BoundaryEntry>
                boundaries = OpsShardReadinessRouteCleanupMaintenanceUpkeepCatalog.items().stream()
                .map(this::entry)
                .toList();
    ...
    private String status(
            List<OpsShardReadinessRouteCleanupMaintenanceDependencyBoundaryMapResponse.BoundaryEntry> boundaries,
            List<String> forbiddenOperations
    ) {
        boolean passed = boundaries.size() == OpsShardReadinessRouteCleanupMaintenanceUpkeepCatalog.items().size()
                ...
                && boundaries.stream().allMatch(boundary -> "read-only-evidence-preview"
                        .equals(boundary.allowedScope()))
