...
final
class OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftInstructionPreflightSupport {
  ...
  OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftInstructionPreflightResponse
    ...
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-slot-count-" + slotCopy.size());
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-passed-slot-count-"
            + passedSlotCount);
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-guard-count-" + guardCopy.size());
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-passed-guard-count-"
            + passedGuardCount);
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-gate-count-" + gateCopy.size());
    checks.add("signed-approval-artifact-draft-instruction-preflight-source-plan-" + SOURCE_PLAN);
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-source-node-authoring-readiness-"
            + SOURCE_NODE_AUTHORING_READINESS_VERSION);
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-source-java-authoring-readiness-"
            + SOURCE_JAVA_AUTHORING_READINESS_VERSION);
    checks.add(
        "signed-approval-artifact-draft-instruction-preflight-no-instruction-artifact-creation");
