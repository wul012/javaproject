...
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoRecords.RehearsalRuntimeShellPostDecisionPlanIntake;
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoRecords.RehearsalRuntimeShellPostDecisionPlanIntakeChecks;
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoRecords.RehearsalRuntimeShellPostDecisionPlanIntakeNecessityProof;
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoRecords.RehearsalRuntimeShellPostDecisionPlanIntakeSideEffectBoundary;
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoRecords.RehearsalRuntimeShellPostDecisionPlanIntakeSourceDecisionRecordEcho;
import java.util.List;

final
class ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoSupport {
  ...
  RehearsalManagedAuditSandboxEndpointCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoReceipt
    ...
    RehearsalRuntimeShellPostDecisionPlanIntakeNecessityProof necessityProof = necessityProof();
  ...
  private static RehearsalRuntimeShellPostDecisionPlanIntakeSideEffectBoundary
      sideEffectBoundary() {
    return ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellEchoBoundaryCatalog
  ...
  private static RehearsalRuntimeShellPostDecisionPlanIntakeChecks checks(
    ...
    boolean continuationSelected =
        "continue-blocked-planning".equals(planIntake.selectedContinuationDecision())
            && planIntake.selectedDecisionOptionCount() == 1;
    List<String> continuationOptionCodes =
        ReleaseApprovalSandboxEndpointCredentialResolverRuntimeShellEchoMetadataCatalog
            .postDecisionPlanIntakeContinuationOptionCodes();
    boolean optionsDocumented =
        planIntake.decisionOptionCount() == continuationOptionCodes.size()
            && planIntake.continuationOptions().stream()
                .map(option -> option.code())
                .toList()
                .equals(continuationOptionCodes);
    boolean implementationRejected =
        planIntake.rejectedRuntimeImplementationOptionCount() == 1
            && planIntake.continuationOptions().stream()
                .anyMatch(
                    option ->
                        "IMPLEMENT_RUNTIME_SHELL_NOW".equals(option.code())
                            && "rejected".equals(option.status()));
    boolean proofHasBlocker = proof.blockerResolved().contains("v300");
    boolean proofHasConsumer =
    ...
    boolean proofDefinesStop =
        proof.stopCondition().contains("credential values")
  ...
  private static EchoWorkflowReadiness readiness(
    ...
    return workflowReadiness(
        workflowStep(
            ...
            checks.sourceDecisionRecordEchoReady()
                && sourceEcho.readyForNodeV300RuntimeShellDecisionRecordUpstreamEchoVerification()),
        ...
        workflowStep(
            ...
            checks.necessityProofComplete() && proof.consumer().contains("Java v136")),
        workflowStep(
            "runtimeImplementationRejectedEchoed", checks.runtimeImplementationOptionRejected()),
        workflowStep(
        ...
        workflowStep(
            ...
            checks.externalRequestStillForbidden() && !boundary.externalRequestSent()),
        ...
        List.of(
            ReleaseApprovalDigestSupport.line(
                ...
                ReleaseApprovalContractConstants
                    .RELEASE_APPROVAL_REHEARSAL_MANAGED_AUDIT_SANDBOX_ENDPOINT_CREDENTIAL_RESOLVER_RUNTIME_SHELL_POST_DECISION_PLAN_INTAKE_ECHO_RECEIPT_VERSION),
            ReleaseApprovalDigestSupport.line(
                "sourceDecisionRecordEchoReceiptVersion", sourceReceipt.receiptVersion()),
            ReleaseApprovalDigestSupport.line(
                "sourceDecisionRecordEchoReceiptDigest", sourceReceipt.receiptDigest()),
  ...
  private static List<String> receiptWarnings(EchoWorkflowReadiness readiness) {
    return ReleaseApprovalDigestSupport.warnings(
        readiness.warningIfMissing(
            "sourceDecisionRecordEchoed", "SOURCE_DECISION_RECORD_ECHO_NOT_READY"),
        readiness.warningIfMissing("nodeV301PlanIntakeEchoed", "NODE_V301_PLAN_INTAKE_NOT_READY"),
        readiness.warningIfMissing(
            "continuationOptionsEchoed", "POST_DECISION_CONTINUATION_OPTIONS_UNSTABLE"),
        readiness.warningIfMissing(
            "necessityProofEchoed", "POST_DECISION_NECESSITY_PROOF_INCOMPLETE"),
        readiness.warningIfMissing(
            "runtimeImplementationRejectedEchoed", "RUNTIME_IMPLEMENTATION_OPTION_NOT_REJECTED"),
        readiness.warningIfMissing(
            "noWriteOrMigrationEchoed", "POST_DECISION_PLAN_WRITE_BOUNDARY_OPEN"));
  }
}
