...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierBoundaryAuditRenderer {

    private OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierBoundaryAuditRenderer() {
    }
    ...
    static OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierRegistryResponse
        ...
        return OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveDigestConsumerPackageVerificationDossierRendererSupport
                .section("Boundary Audits", entries.stream()
                        .map(entry -> entry.code()
                                + " | lockedBehavior=" + entry.lockedBehavior()
                                + " | evidence=" + entry.auditEvidence()
                                + " | status=" + entry.status())
                        .toList());
    }
}
