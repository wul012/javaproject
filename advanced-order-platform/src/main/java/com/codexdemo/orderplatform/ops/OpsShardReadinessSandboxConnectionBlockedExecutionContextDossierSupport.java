...
final class OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierSupport {
    ...
    static OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse response(
            ...
            List<OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.ContextField>
                    contextFields,
            List<OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.NormalizationRule>
                    normalizationRules,
            ...
            List<OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.VerificationGate>
                    verificationGates,
    ...
    private static List<String> checks(
        ...
        result.add("sandbox-connection-blocked-execution-context-dossier-owner-plan-" + NODE_OWNER_PLAN);
        result.add("sandbox-connection-blocked-execution-context-dossier-java-context-" + JAVA_CONTEXT_EVIDENCE_VERSION);
        sourceReceipts.stream().findFirst().ifPresent(source -> {
            result.add("sandbox-connection-blocked-execution-context-dossier-source-receipt-"
                    + source.receiptVersion());
            result.add("sandbox-connection-blocked-execution-context-dossier-consumes-"
                    + source.consumedNodeVersion());
            result.add("sandbox-connection-blocked-execution-context-dossier-next-node-"
                    + source.nextNodeVersion());
            result.add("sandbox-connection-blocked-execution-context-dossier-production-audit-"
                    + source.nodeMayTreatAsProductionAuditRecord());
        });
        result.add("sandbox-connection-blocked-execution-context-dossier-context-fields-" + contextFields.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-normalization-rules-"
                + normalizationRules.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-precondition-evidence-"
                + preconditionEvidence.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-boundaries-" + boundarySnapshots.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-execution-guards-"
                + executionGuards.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-warning-echoes-" + warningEchoes.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-downstream-gates-"
                + downstreamIntakeGates.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-verification-gates-"
                + verificationGates.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-handoff-notes-" + handoffNotes.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-markdown-sections-"
                + markdownSections.size());
        result.add("sandbox-connection-blocked-execution-context-dossier-all-execution-guards-passed-"
                + allExecutionGuardsPassed(executionGuards));
        result.add("sandbox-connection-blocked-execution-context-dossier-warnings-archived-"
                + warningEchoes.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.WarningEcho::archived));
    ...
    static String status(
        ...
        boolean passed = sourceReceipts.size() == EXPECTED_SOURCE_RECEIPT_COUNT
                ...
                && !source.nodeMayTreatAsProductionAuditRecord())
                && contextFields.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.ContextField::normalized)
                && normalizationRules.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.NormalizationRule::enforced)
                && preconditionEvidence.stream().allMatch(evidence -> evidence.required() && evidence.present())
                && boundarySnapshots.stream().allMatch(snapshot -> snapshot.required() && snapshot.closed())
                && allExecutionGuardsPassed(executionGuards)
                && warningEchoes.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.WarningEcho::archived)
                && downstreamIntakeGates.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.DownstreamIntakeGate::ready)
                && verificationGates.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.VerificationGate::passed)
                && handoffNotes.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.HandoffNote::ready);
    ...
    private static boolean allExecutionGuardsPassed(
        ...
        return executionGuards.stream().allMatch(
                OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.ExecutionGuard::passed);
    }
}
