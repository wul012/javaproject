...
public class OpsShardReadinessRouteCleanupCompletionIndexService {
    ...
    public OpsShardReadinessRouteCleanupCompletionIndexResponse index() {
        OpsShardReadinessRouteCleanupReviewerPacketResponse reviewerPacket = reviewerPacketService.packet();
        OpsShardReadinessRouteCleanupTransitionBriefResponse transitionBrief = transitionBriefService.brief();
        ...
        List<OpsShardReadinessRouteCleanupCompletionIndexResponse.CompletionEndpoint> endpoints = List.of(
                endpoint("reviewer-packet", OpsShardReadinessRouteCleanupReviewerPacketService.ENDPOINT, "review"),
                endpoint("transition-brief", OpsShardReadinessRouteCleanupTransitionBriefService.ENDPOINT, "handoff"),
                endpoint("final-verification",
                        OpsShardReadinessRouteCleanupFinalVerificationService.ENDPOINT,
                        "verification"),
                endpoint("final-archive-plan",
                        OpsShardReadinessRouteCleanupFinalArchivePlanService.ENDPOINT,
                        "archive"),
                endpoint("third-run-closeout",
                        OpsShardReadinessRouteCleanupThirdRunCloseoutService.ENDPOINT,
                        "closeout"),
                endpoint("completion-index", ENDPOINT, "index")
        );
        List<OpsShardReadinessRouteCleanupCompletionIndexResponse.StatusSignal> signals = List.of(
        ...
        boolean passed = signals.stream()
                .allMatch(signal -> signal.status().equals("passed") || signal.evidence().equals("true"));
