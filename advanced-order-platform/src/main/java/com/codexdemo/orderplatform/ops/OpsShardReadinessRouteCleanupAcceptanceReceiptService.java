...
public class OpsShardReadinessRouteCleanupAcceptanceReceiptService {
    ...
    public OpsShardReadinessRouteCleanupAcceptanceReceiptResponse receipt() {
        ...
        OpsShardReadinessRouteCleanupExtendedCloseoutResponse closeout = extendedCloseoutService.closeout();
        List<OpsShardReadinessRouteCleanupAcceptanceReceiptResponse.AcceptedCriterion> criteria = List.of(
                criterion("audit-trail-passed", auditTrail.auditTrailEndpoint() + ":" + auditTrail.status()),
                criterion("extended-closeout-passed", closeout.closeoutEndpoint() + ":" + closeout.status()),
                criterion("read-only-boundary-held", OpsShardReadinessRouteCleanupEvidenceAnalyzer.boundaryStatus()),
                criterion("source-plan-anchored", auditTrail.sourcePlan()),
                criterion("execution-remains-disabled", String.valueOf(!auditTrail.executionAllowed()
                        && !closeout.executionAllowed()))
        );
        boolean accepted = auditTrail.status().equals("passed")
