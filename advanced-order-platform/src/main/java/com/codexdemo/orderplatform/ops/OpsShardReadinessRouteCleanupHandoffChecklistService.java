...
public class OpsShardReadinessRouteCleanupHandoffChecklistService {
    ...
    public OpsShardReadinessRouteCleanupHandoffChecklistResponse checklist() {
        ...
        OpsShardReadinessRouteCleanupBoundaryMatrixResponse boundaryMatrix = boundaryMatrixService.matrix();
        List<OpsShardReadinessRouteCleanupHandoffChecklistResponse.CheckItem> checks = List.of(
                check("catalog-continuity", "java", OpsShardReadinessRouteCleanupEvidenceAnalyzer.versionsAreContinuous(),
                        "catalog entries are continuous from v306 through current Java version"),
                check("phase-summary-ready", "java", phaseSummary.status().equals("passed"),
                        "phase summary profile " + phaseSummary.summaryProfile()),
                check("boundary-matrix-fail-closed", "java", boundaryMatrix.status().equals("passed"),
                        "boundary matrix profile " + boundaryMatrix.matrixProfile()),
                check("execution-disabled", "java", !boundaryMatrix.executionAllowed(),
                        "route cleanup suite remains read-only"),
                check("sibling-start-disabled", "node/java/mini-kv", true,
        ...
        return new OpsShardReadinessRouteCleanupHandoffChecklistResponse(
                ...
                checks.size(),
                checks,
                checks.stream().allMatch(OpsShardReadinessRouteCleanupHandoffChecklistResponse.CheckItem::passed)
                        ? "passed"
                        : "blocked"
        );
    }
