...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffRegistrySupport {
    ...
    static final String HANDOFF_STATE =
            "minimal-read-only-gate-operator-ci-handoff-ready";
    ...
    static OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffRegistryResponse response(
            ...
            List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffRegistryResponse.SourceArchiveSnapshot>
        ...
        int readyOperatorLaneCount = (int) operatorLaneCopy.stream()
                .filter(OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffRegistryResponse
                        .OperatorLane::ready)
                .count();
        int passedCiBatchCount = (int) ciBatchCopy.stream()
                .filter(OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffRegistryResponse
                        .CiBatchPlan::passed)
                .count();
        int lockedBoundaryCount = (int) boundaryLockCopy.stream()
                .filter(OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffRegistryResponse
                        .BoundaryLock::locked)
                .count();
        int passedScorecardEntryCount = (int) scorecardCopy.stream()
        ...
        checks.add("minimal-read-only-gate-operator-ci-handoff-required-archive-"
                + REQUIRED_ARCHIVE_VERIFICATION_PLAN);
        checks.add("minimal-read-only-gate-operator-ci-handoff-recommended-plan-"
                + RECOMMENDED_OPERATOR_PLAN);
        checks.add("minimal-read-only-gate-operator-ci-handoff-source-archive-version-"
                + sourceArchive.version());
        checks.add("minimal-read-only-gate-operator-ci-handoff-source-archive-count-"
                + sourceArchiveSnapshotCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-lane-count-" + operatorLaneCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-ready-lane-count-"
                + readyOperatorLaneCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-ci-batch-count-" + ciBatchCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-passed-ci-batch-count-"
                + passedCiBatchCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-boundary-lock-count-"
                + boundaryLockCopy.size());
        checks.add("minimal-read-only-gate-operator-ci-handoff-locked-boundary-count-"
                + lockedBoundaryCount);
        checks.add("minimal-read-only-gate-operator-ci-handoff-scorecard-count-"
                + scorecardCopy.size());
        ...
        String status = readyOperatorLaneCount == operatorLaneCopy.size()
