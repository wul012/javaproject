...
final class OpsShardReadinessMinimalReadOnlyGateExecutionRegistrySupport {
  ...
  static OpsShardReadinessMinimalReadOnlyGateExecutionRegistryResponse response(
      ...
      List<OpsShardReadinessMinimalReadOnlyGateExecutionRegistryResponse.BoundaryRule>
          boundaryRules,
      List<OpsShardReadinessMinimalReadOnlyGateExecutionRegistryResponse.CiBatch> ciBatches,
    ...
    int passedReadTargetCount =
        (int) readTargetCopy.stream().filter(target -> "passed".equals(target.status())).count();
    int passedGateCheckCount =
        (int)
            gateCheckCopy.stream()
                .filter(
                    OpsShardReadinessMinimalReadOnlyGateExecutionRegistryResponse.GateCheck::passed)
                .count();
    ...
    checks.add(
        "minimal-read-only-gate-execution-passed-read-target-count-" + passedReadTargetCount);
    ...
    checks.add(
        "minimal-read-only-gate-execution-denied-boundary-rule-count-" + deniedBoundaryRuleCount);
    checks.add("minimal-read-only-gate-execution-ci-batch-count-" + ciBatchCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-requirement-count-"
            + archiveRequirementCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-operator-handoff-count-" + operatorHandoffCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-markdown-section-count-" + markdownSectionCopy.size());
    ...
    String status =
        passedReadTargetCount == readTargetCopy.size()
