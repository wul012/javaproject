...
final class OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierRenderer {
    ...
    static List<OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.MarkdownSection> render(
        ...
        return List.of(
                section("Source Receipt", sourceReceipts.stream()
                        ...
                        .toList()),
                section("Context Fields", contextFields.stream()
                        .map(field -> "- " + field.name() + " source=" + field.source())
                        .toList()),
                section("Precondition Evidence", preconditionEvidence.stream()
                        .map(evidence -> "- " + evidence.id() + " present=" + evidence.present())
                        .toList()),
                section("Boundaries", boundarySnapshots.stream()
                        .map(boundary -> "- " + boundary.name() + " closed=" + boundary.closed())
                        .toList()),
                section("Execution Guards", executionGuards.stream()
                        .map(guard -> "- " + guard.name() + " passed=" + guard.passed())
                        .toList()),
                section("Warnings", warningEchoes.stream()
                        .map(warning -> "- " + warning.source() + " " + warning.warning())
                        .toList()),
                section("Downstream Intake", downstreamIntakeGates.stream()
    ...
    private static OpsShardReadinessSandboxConnectionBlockedExecutionContextDossierResponse.MarkdownSection section(
            String heading,
            List<String> lines
    ) {
