...
final class OpsShardReadinessMinimalReadOnlyGateExecutionArchiveVerificationRegistrySupport {
  ...
  static final String ARCHIVE_STATE = "minimal-read-only-gate-execution-archive-verification-ready";
  ...
  static OpsShardReadinessMinimalReadOnlyGateExecutionArchiveVerificationRegistryResponse response(
    ...
    int deniedBoundaryVerificationCount =
        (int)
            boundaryVerificationCopy.stream()
                .filter(
                    OpsShardReadinessMinimalReadOnlyGateExecutionArchiveVerificationRegistryResponse
    ...
    checks.add(
        "minimal-read-only-gate-execution-archive-source-registry-count-"
            + sourceRegistrySnapshotCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-artifact-count-"
            + artifactVerificationCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-passed-artifact-count-"
            + passedArtifactVerificationCount);
    checks.add(
        "minimal-read-only-gate-execution-archive-read-target-count-"
            + readTargetVerificationCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-passed-read-target-count-"
            + passedReadTargetVerificationCount);
    checks.add(
        "minimal-read-only-gate-execution-archive-gate-check-count-"
            + gateCheckVerificationCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-passed-gate-check-count-"
            + passedGateCheckVerificationCount);
    checks.add(
        "minimal-read-only-gate-execution-archive-boundary-count-"
            + boundaryVerificationCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-denied-boundary-count-"
            + deniedBoundaryVerificationCount);
    checks.add(
        "minimal-read-only-gate-execution-archive-ci-batch-count-"
            + ciBatchVerificationCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-operator-handoff-count-"
            + operatorHandoffVerificationCopy.size());
    checks.add("minimal-read-only-gate-execution-archive-scorecard-count-" + scorecardCopy.size());
    checks.add(
        "minimal-read-only-gate-execution-archive-markdown-section-count-"
    ...
    String status =
        passedArtifactVerificationCount == artifactVerificationCopy.size()
