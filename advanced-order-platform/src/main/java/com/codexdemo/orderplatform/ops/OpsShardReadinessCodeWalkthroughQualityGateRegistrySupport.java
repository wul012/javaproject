...
final class OpsShardReadinessCodeWalkthroughQualityGateRegistrySupport {
    ...
    static OpsShardReadinessCodeWalkthroughQualityGateRegistryResponse response(
        ...
        int deniedBoundaryRuleCount = (int) boundaryRuleCopy.stream()
                ...
                .count();
        boolean rulesRequired = versionRuleCopy.stream()
                .allMatch(OpsShardReadinessCodeWalkthroughQualityGateRegistryResponse
                        .VersionRule::required);
        boolean anchorsRuntimeFree = evidenceAnchorCopy.stream()
                .allMatch(OpsShardReadinessCodeWalkthroughQualityGateRegistryResponse
                        .EvidenceAnchor::runtimeFree);
        boolean checklistsBlockRelease = reviewChecklistCopy.stream()
                .allMatch(OpsShardReadinessCodeWalkthroughQualityGateRegistryResponse
                        .ReviewChecklist::blocksRelease);
        boolean statusPassed = versionRuleCopy.size() == EXPECTED_VERSION_RULE_COUNT
        ...
        checks.add("code-walkthrough-quality-gate-explanation-rubric-count-"
                + explanationRubricCopy.size());
        checks.add("code-walkthrough-quality-gate-evidence-anchor-count-" + evidenceAnchorCopy.size());
        checks.add("code-walkthrough-quality-gate-review-checklist-count-"
                + reviewChecklistCopy.size());
        checks.add("code-walkthrough-quality-gate-boundary-rule-count-" + boundaryRuleCopy.size());
        checks.add("code-walkthrough-quality-gate-denied-boundary-rule-count-"
                + deniedBoundaryRuleCount);
        checks.add("code-walkthrough-quality-gate-markdown-section-count-"
