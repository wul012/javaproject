...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutBoundaryCatalog {
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutResponse.BoundaryAssertion> assertions(
            OpsShardReadinessReleaseAcceptanceRoutePathSplitResponse source
    ) {
        return source.boundaryGuards().stream()
                .map(guard -> new OpsShardReadinessReleaseAcceptanceRoutePathSplitCloseoutResponse.BoundaryAssertion(
                        guard.boundary(),
                        guard.locked(),
                        guard.evidence()
                ))
