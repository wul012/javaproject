...
public class OpsShardReadinessRouteCleanupHandoffBundleService {
    ...
    public OpsShardReadinessRouteCleanupHandoffBundleResponse bundle() {
        ...
        List<OpsShardReadinessRouteCleanupHandoffBundleResponse.BundleComponent> components = List.of(
                component("consumer-packet", OpsShardReadinessRouteCleanupConsumerPacketService.ENDPOINT, packet.status()),
                component("ci-evidence", OpsShardReadinessRouteCleanupCiEvidenceService.ENDPOINT, ciEvidence.status()),
                component("regression-guard", OpsShardReadinessRouteCleanupRegressionGuardService.ENDPOINT, guard.status())
        );
        boolean passed = components.stream()
                .allMatch(component -> component.status().equals("passed"));
    ...
    private OpsShardReadinessRouteCleanupHandoffBundleResponse.BundleComponent component(
        ...
        return new OpsShardReadinessRouteCleanupHandoffBundleResponse.BundleComponent(name, endpoint, status);
    }
}
