...
final class OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveArtifactVerificationCatalog {

    private OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveArtifactVerificationCatalog() {
    }
    ...
    static List<OpsShardReadinessMinimalReadOnlyGateOperatorCiHandoffArchiveVerificationRegistryResponse
        ...
        return List.of(
                artifact("source-response-json", source.version(), source.endpoint(),
                        "passed".equals(source.status())),
                artifact("markdown-section-rendering", source.version(),
                        ...
                        source.markdownSections().size()
                                ...
                                .EXPECTED_MARKDOWN_SECTION_COUNT - 1),
                artifact("operator-lane-plan", source.version(),
                        "operator-lanes=" + source.readyOperatorLaneCount() + "/"
                                + source.operatorLaneCount(),
                        source.readyOperatorLaneCount() == source.operatorLaneCount()),
                artifact("ci-batch-plan", source.version(),
                        "ci-batches=" + source.passedCiBatchCount() + "/" + source.ciBatchCount(),
                        source.passedCiBatchCount() == source.ciBatchCount()),
                artifact("boundary-lock-plan", source.version(),
                        "boundary-locks=" + source.lockedBoundaryCount() + "/"
                                + source.boundaryLockCount(),
                        source.lockedBoundaryCount() == source.boundaryLockCount()),
                artifact("source-scorecard-summary", source.version(),
