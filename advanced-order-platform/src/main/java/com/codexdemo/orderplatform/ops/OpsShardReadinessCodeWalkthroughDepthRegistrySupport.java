...
final class OpsShardReadinessCodeWalkthroughDepthRegistrySupport {
    ...
    static final String REGISTRY_STATE =
            "chinese-longform-walkthrough-depth-enforced-from-v1774";
    ...
    static OpsShardReadinessCodeWalkthroughDepthRegistryResponse response(
        ...
        int deniedBoundaryRuleCount = (int) boundaryRuleCopy.stream()
                ...
                .count();
        boolean depthRulesRequired = depthRuleCopy.stream()
                .allMatch(OpsShardReadinessCodeWalkthroughDepthRegistryResponse
                        .DepthRule::required);
        boolean languageRulesRequired = languageRuleCopy.stream()
                .allMatch(OpsShardReadinessCodeWalkthroughDepthRegistryResponse
                        .LanguageRule::required);
        boolean verificationRequired = verificationStepCopy.stream()
                .allMatch(OpsShardReadinessCodeWalkthroughDepthRegistryResponse
                        .VerificationStep::required);
        boolean minimumIsEnforced = depthRuleCopy.stream()
                .allMatch(rule -> rule.minimumChineseCharacters()
                        >= MINIMUM_CHINESE_CHARACTER_COUNT);
        boolean statusPassed = depthRuleCopy.size() == EXPECTED_DEPTH_RULE_COUNT
        ...
        checks.add("code-walkthrough-depth-minimum-chinese-characters-"
                + MINIMUM_CHINESE_CHARACTER_COUNT);
        ...
        checks.add("code-walkthrough-depth-denied-boundary-rule-count-"
                + deniedBoundaryRuleCount);
        checks.add("code-walkthrough-depth-verification-step-count-"
