...
public class OpsShardReadinessRouteCleanupRegressionGuardService {
  ...
  static final String ENDPOINT = RouteCleanupRoutes.BASE_PATH + RouteCleanupRoutes.REGRESSION_GUARD;
  ...
  public OpsShardReadinessRouteCleanupRegressionGuardResponse guard() {
    ...
    List<String> endpoints =
        manifest.endpoints().stream()
            .map(OpsShardReadinessRouteCleanupEndpointManifestResponse.EndpointEntry::endpoint)
            .toList();
    List<OpsShardReadinessRouteCleanupRegressionGuardResponse.GuardCheck> guards =
        List.of(
            guard(
                "version-continuity",
            ...
            guard(
                ...
                endpoints.contains(OpsShardReadinessRouteCleanupConsumerPacketService.ENDPOINT)
                    && endpoints.contains(
