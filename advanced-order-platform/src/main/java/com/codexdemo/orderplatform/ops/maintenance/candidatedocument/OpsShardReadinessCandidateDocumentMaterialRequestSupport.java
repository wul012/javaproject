...
final class OpsShardReadinessCandidateDocumentMaterialRequestSupport {
  ...
  static OpsShardReadinessCandidateDocumentMaterialRequestResponse response(
      ...
      List<OpsShardReadinessCandidateDocumentMaterialRequestResponse.AcceptanceCheck>
          acceptanceChecks,
    ...
    int passedRequestItemCount =
        (int) requestCopy.stream().filter(item -> "passed".equals(item.status())).count();
    int passedAcceptanceCheckCount =
        (int) acceptanceCopy.stream().filter(check -> "passed".equals(check.status())).count();
    List<String> checks = new ArrayList<>();
    checks.add("candidate-document-material-request-source-plan-" + SOURCE_PLAN);
    checks.add(
        "candidate-document-material-request-source-node-" + SOURCE_NODE_INTAKE_PACKET_VERSION);
    checks.add(
        "candidate-document-material-request-source-java-intake-packet-" + sourcePacket.version());
    ...
    checks.add(
        "candidate-document-material-request-acceptance-check-count-" + acceptanceCopy.size());
    checks.add(
        "candidate-document-material-request-source-slot-count-" + sourcePacket.intakeSlotCount());
    checks.add(
        "candidate-document-material-request-source-guard-count-"
            + sourcePacket.intakeGuardCount());
    checks.add(
        "candidate-document-material-request-field-count-"
