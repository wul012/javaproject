...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageNextChangeCatalog {

    private OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageNextChangeCatalog() {
    }
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageResponse
        ...
        return List.of(
                rule("new-route-path", "route-path catalog then release acceptance route group", "route-owner",
                        ready),
                rule("new-consumer", "consumer catalog", "handoff-owner", ready),
                rule("new-ci-gate", "CI catalog and renderer", "ci-owner", ready),
                rule("new-boundary", "boundary catalog and runtime boundary package", "ops-boundary-owner",
                        ready),
                rule("source-plan-roll", "support constants and source catalog", "sibling-plan-owner", ready),
                rule("markdown-copy-change", "section renderer for the affected concern", "renderer-owner",
                        ready)
        );
    }
