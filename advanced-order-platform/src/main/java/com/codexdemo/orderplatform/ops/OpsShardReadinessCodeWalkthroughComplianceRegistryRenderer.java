...
final class OpsShardReadinessCodeWalkthroughComplianceRegistryRenderer {
    ...
    static List<OpsShardReadinessCodeWalkthroughComplianceRegistryResponse.MarkdownSection>
            render(
    ...
    private static OpsShardReadinessCodeWalkthroughComplianceRegistryResponse.MarkdownSection
        ...
        versions.forEach(version -> lines.add(version.javaVersion()
                + " | "
                + version.tag()
                + " | "
                + version.focus()
    ...
    private static OpsShardReadinessCodeWalkthroughComplianceRegistryResponse.MarkdownSection
        ...
        requiredHeadings.forEach(heading -> lines.add(heading.order()
                + ". "
                + heading.heading()
                + " -> "
                + heading.intent()));
    ...
    private static OpsShardReadinessCodeWalkthroughComplianceRegistryResponse.MarkdownSection
        ...
        archiveRanges.forEach(range -> lines.add(range.name()
                + " | "
                + range.directory()
                + " | "
                + range.versionRange()
    ...
    private static OpsShardReadinessCodeWalkthroughComplianceRegistryResponse.MarkdownSection
        ...
        documentationRules.forEach(rule -> lines.add(rule.code()
                + " | owner="
                + rule.owner()
                + " | required="
                + rule.required()
    ...
    private static OpsShardReadinessCodeWalkthroughComplianceRegistryResponse.MarkdownSection
        ...
        testCoverages.forEach(test -> lines.add(test.testClass()
                + " | "
                + test.scope()
                + " | blocks-release="
                + test.blocksRelease()
