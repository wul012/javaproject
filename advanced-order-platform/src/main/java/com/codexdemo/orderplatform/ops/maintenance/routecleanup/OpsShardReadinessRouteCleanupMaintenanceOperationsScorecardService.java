...
public class OpsShardReadinessRouteCleanupMaintenanceOperationsScorecardService {
  ...
  public static final String ENDPOINT =
      RouteCleanupRoutes.BASE_PATH + RouteCleanupRoutes.MAINTENANCE_OPERATIONS_SCORECARD;
        ...
        dimensions =
            List.of(
                dimension(
                    ...
                    handoff.endpoint(),
                    25,
                    "accepted-section-count-" + handoff.acceptedSectionCount(),
                    handoff.status()),
                dimension(
                    "dependency-boundary",
                    boundary.endpoint(),
                    25,
                    "boundary-entry-count-" + boundary.boundaryEntryCount(),
                    boundary.status()),
                dimension(
                    "archive-retention",
                    archive.endpoint(),
                    25,
                    "next-review-version-" + archive.nextReviewVersion(),
                    archive.status()),
                dimension(
                    "test-evidence",
                    tests.endpoint(),
                    25,
                    "covered-entry-count-" + tests.coveredEntryCount(),
                    tests.status()));
    int passed =
        (int) dimensions.stream().filter(dimension -> "passed".equals(dimension.status())).count();
    int score =
        dimensions.stream()
            .filter(dimension -> "passed".equals(dimension.status()))
            .mapToInt(
                OpsShardReadinessRouteCleanupMaintenanceOperationsScorecardResponse
                        .ScorecardDimension
                    ::weight)
            .sum();
    List<String> checks =
  ...
  private OpsShardReadinessRouteCleanupMaintenanceOperationsScorecardResponse.ScorecardDimension
      dimension(String name, String sourceEndpoint, int weight, String evidence, String status) {
    return new OpsShardReadinessRouteCleanupMaintenanceOperationsScorecardResponse
        .ScorecardDimension(name, sourceEndpoint, weight, evidence, status);
  }
}
