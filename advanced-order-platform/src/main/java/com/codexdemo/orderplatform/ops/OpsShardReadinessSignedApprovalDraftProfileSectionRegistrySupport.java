...
final class OpsShardReadinessSignedApprovalDraftProfileSectionRegistrySupport {
    ...
    static OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse response(
            ...
            List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.DraftSectionSource> sources,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.DraftProfileSection> sections,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.FieldEntry> fieldEntries,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.RenderedSection> renderedSections,
            List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.RouteFieldLock> routeFieldLocks,
            List<String> gates,
        ...
        int lockedRouteFieldCount = lockCopy.stream()
                .mapToInt(OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse
                        .RouteFieldLock::lockedFieldCount)
                .sum();
        List<String> checks = new ArrayList<>();
        checks.add("signed-approval-draft-profile-section-registry-source-plan-" + SOURCE_PLAN);
        checks.add("signed-approval-draft-profile-section-registry-source-node-"
        ...
        checks.add("signed-approval-draft-profile-section-registry-source-route-count-" + sourceCopy.size());
        checks.add("signed-approval-draft-profile-section-registry-section-count-" + sectionCopy.size());
        checks.add("signed-approval-draft-profile-section-registry-rendered-section-count-"
                + renderedCopy.size());
        checks.add("signed-approval-draft-profile-section-registry-field-entry-count-" + fieldCopy.size());
        checks.add("signed-approval-draft-profile-section-registry-route-field-lock-count-"
                + lockCopy.size());
        checks.add("signed-approval-draft-profile-section-registry-locked-route-field-count-"
        ...
        return new OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse(
                ...
                isComplete(moduleCopy, sourceCopy, sectionCopy, fieldCopy, renderedCopy,
                        lockCopy, lockedRouteFieldCount, gateCopy)
                        ? "passed"
                        : "blocked"
        );
    }
