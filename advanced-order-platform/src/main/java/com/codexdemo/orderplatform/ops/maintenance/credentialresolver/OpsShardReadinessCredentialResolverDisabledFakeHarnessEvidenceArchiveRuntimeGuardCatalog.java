...
final
class OpsShardReadinessCredentialResolverDisabledFakeHarnessEvidenceArchiveRuntimeGuardCatalog {
  ...
  static List<
    ...
    return List.of(
        ...
        guard(
            "provider-client",
            "secretProviderInstantiated=false,resolverClientInstantiated=false",
            !boundary.secretProviderInstantiated() && !boundary.resolverClientInstantiated()),
        guard(
            "ledger-write",
            "approvalLedgerWritten=false,managedAuditStoreWritten=false",
            !boundary.approvalLedgerWritten() && !boundary.managedAuditStoreWritten()),
        guard(
            "schema-sql",
            "sqlExecuted=false,schemaMigrationExecuted=false",
            !boundary.sqlExecuted() && !boundary.schemaMigrationExecuted()),
        guard(
            "auto-start",
            "automaticUpstreamStart=false,javaStartedNodeOrMiniKv=false",
