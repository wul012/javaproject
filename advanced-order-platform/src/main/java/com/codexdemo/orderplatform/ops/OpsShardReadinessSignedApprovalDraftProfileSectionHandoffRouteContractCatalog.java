...
final class OpsShardReadinessSignedApprovalDraftProfileSectionHandoffRouteContractCatalog {
    ...
    routeContracts(
            List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.RouteFieldLock> locks
    ) {
        return locks.stream()
                .map(lock -> new OpsShardReadinessSignedApprovalDraftProfileSectionHandoffResponse
                        .RouteContract(
                                lock.sectionCode(),
                                lock.endpoint(),
                                lock.profile(),
