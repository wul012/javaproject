...
final class OpsShardReadinessSignedApprovalDraftProfileSectionCatalog {
    ...
    static List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.DraftProfileSection> sections(
            List<OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse.DraftSectionSource> sources
    ) {
        return sources.stream()
                .map(source -> new OpsShardReadinessSignedApprovalDraftProfileSectionRegistryResponse
                        .DraftProfileSection(
                                source.order(),
                                source.code() + "-section",
                                heading(source.code()),
    ...
    private static String heading(String code) {
        return switch (code) {
            case "signed-approval-artifact-draft-preflight" ->
                    "Signed Approval Artifact Draft Preflight";
            case "signed-approval-artifact-draft-readiness" ->
