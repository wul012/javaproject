...
public class OpsShardReadinessRouteCleanupMaintenanceTestBudgetPlanService {
    ...
    public OpsShardReadinessRouteCleanupMaintenanceTestBudgetPlanResponse plan() {
        List<OpsShardReadinessRouteCleanupMaintenanceTestBudgetPlanResponse.TestStep> steps = List.of(
                step("focused-maintenance-services", "new maintenance service tests", "passed"),
                step("maintenance-route-integration", "maintenance MockMvc integration tests", "passed"),
    ...
    private String status(
        ...
        boolean passed = steps.size() == 5
                && steps.stream().noneMatch(OpsShardReadinessRouteCleanupMaintenanceTestBudgetPlanResponse
                        .TestStep::startsJavaService)
                && steps.stream().noneMatch(OpsShardReadinessRouteCleanupMaintenanceTestBudgetPlanResponse
                        .TestStep::startsMiniKvService)
                && steps.stream().noneMatch(OpsShardReadinessRouteCleanupMaintenanceTestBudgetPlanResponse
                        .TestStep::startsNodeService);
