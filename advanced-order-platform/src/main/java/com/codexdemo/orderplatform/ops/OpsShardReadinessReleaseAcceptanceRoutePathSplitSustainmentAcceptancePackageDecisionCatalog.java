...
final class OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageDecisionCatalog {

    private OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageDecisionCatalog() {
    }
    ...
    static List<OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageResponse
        ...
        return List.of(
                decision("accept-sustainment-registry", "release-acceptance-maintainer",
                        "v1604 sustainment registry passed", "passed".equals(source.status())),
                decision("freeze-stable-route-delegate", "route-owner",
                        "stable-route-delegate ownership rule is enforced",
                        ownershipHeld(source, "stable-route-delegate")),
                decision("require-catalog-before-route", "catalog-owner",
                        "catalog-ownership rule is enforced before endpoint growth",
                        ownershipHeld(source, "catalog-ownership")),
                decision("require-renderer-split", "renderer-owner",
                        "renderer-ownership rule keeps markdown sections separated",
                        ownershipHeld(source, "renderer-ownership")),
                decision("keep-runtime-disabled", "ops-boundary-owner",
                        "package remains read-only and execution is not allowed",
                        source.readOnly() && !source.executionAllowed()),
                decision("parallel-node-no-fresh-evidence", "sibling-plan-owner",
