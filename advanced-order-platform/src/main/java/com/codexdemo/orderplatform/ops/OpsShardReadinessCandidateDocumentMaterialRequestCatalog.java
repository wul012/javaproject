...
final class OpsShardReadinessCandidateDocumentMaterialRequestCatalog {
    ...
    static List<OpsShardReadinessCandidateDocumentMaterialRequestResponse.ModuleEntry> modules() {
        return List.of(
                ...
                module(207, "material-request-artifacts", "names archive references without accepting material"),
                module(208, "material-request-route", "exposes the route and no-material closeout evidence")
        );
    }
    ...
    static List<OpsShardReadinessCandidateDocumentMaterialRequestResponse.RequestItem> requestItems(
        ...
        List<OpsShardReadinessCandidateDocumentMaterialRequestResponse.RequestItem> items = new ArrayList<>();
        sourcePacket.intakeSlots().forEach(slot -> items.add(item(
                "material-slot-" + slot.code(),
                slot.code(),
                "intake-slot-material",
                "reviewer identity, source uri",
                "Request external reviewed material for " + slot.code())));
        sourcePacket.intakeGuards().forEach(guard -> items.add(item(
                "material-guard-" + guard.slotCode(),
                guard.code(),
                "guard-attestation",
                "absence attestation, boundary confirmation",
                "Request reviewer attestation for " + guard.slotCode())));
        items.add(item("reviewer-identity-request", "external-material", "external-material",
                "reviewer identity, review timestamp", "Request reviewer identity and review timestamp."));
        items.add(item("source-uri-digest-request", "external-material", "external-material",
                "source uri, digest", "Request source URI and digest lineage."));
        items.add(item("canonical-body-field-table-request", "external-material", "external-material",
                "canonical body, field table", "Request canonical body and material field table."));
        items.add(item("comparison-signature-request", "external-material", "external-material",
                "comparison binding, signature attestation",
                "Request comparison binding and detached signature attestation."));
        items.add(item("redaction-archive-closeout-request", "external-material", "external-material",
                "redaction log, archive index, absence attestations",
                "Request redaction log, archive index, and absence attestations."));
        return List.copyOf(items);
    }

    static List<OpsShardReadinessCandidateDocumentMaterialRequestResponse.AcceptanceCheck> acceptanceChecks(
            List<OpsShardReadinessCandidateDocumentMaterialRequestResponse.RequestItem> requestItems
    ) {
        return requestItems.stream()
                .map(item -> new OpsShardReadinessCandidateDocumentMaterialRequestResponse.AcceptanceCheck(
                        item.code() + "-acceptance-check",
                        item.code(),
                        "reject-material-request-" + item.code(),
                        "Reject material request until external reviewed material satisfies " + item.instruction(),
                        "fail-closed",
                        "passed"))
                .toList();
    }
    ...
    static List<OpsShardReadinessCandidateDocumentMaterialRequestResponse.Artifact> artifacts() {
        return List.of(
                ...
                artifact("acceptance-checks", "e/1152/material/candidate-document-material-acceptance-checks.json"),
                artifact("disabled-boundaries", "e/1152/policy/candidate-document-material-request-boundaries.json"),
    ...
    static List<String> gates() {
        return IntStream.rangeClosed(1, OpsShardReadinessCandidateDocumentMaterialRequestSupport.EXPECTED_GATE_COUNT)
    ...
    private static OpsShardReadinessCandidateDocumentMaterialRequestResponse.Artifact artifact(
        ...
        return new OpsShardReadinessCandidateDocumentMaterialRequestResponse.Artifact(
                code,
                reference,
                "material request evidence only",
                "passed"
        );
    }
}
