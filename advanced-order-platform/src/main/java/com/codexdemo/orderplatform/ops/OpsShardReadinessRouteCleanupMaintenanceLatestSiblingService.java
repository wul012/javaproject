...
public class OpsShardReadinessRouteCleanupMaintenanceLatestSiblingService {
    ...
    public OpsShardReadinessRouteCleanupMaintenanceLatestSiblingResponse report() {
        ...
        List<String> sourcePlans = entries.stream()
                .map(OpsShardReadinessRouteCleanupEvidenceResponse.Entry::sourceNodePlan)
                .distinct()
                .toList();
        List<String> evidencePaths = entries.stream()
                .map(OpsShardReadinessRouteCleanupEvidenceResponse.Entry::evidencePath)
                .toList();
        int liveSmokeCount = (int) entries.stream()
                .filter(entry -> entry.phase().contains("live-smoke"))
                .count();
    ...
    private String status(
        ...
        boolean passed = entries.size() == 12
                && entries.getFirst().javaVersion() == 306
                && entries.getLast().javaVersion() == 317
                && liveSmokeCount == 6
                && sourcePlans.contains("Node v549")
                && entries.stream().allMatch(OpsShardReadinessRouteCleanupEvidenceResponse.Entry::readOnly)
                && entries.stream().noneMatch(OpsShardReadinessRouteCleanupEvidenceResponse.Entry::executionAllowed);
