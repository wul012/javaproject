...
public class OpsShardReadinessRouteCleanupPostPushCloseoutService {
    ...
    public OpsShardReadinessRouteCleanupPostPushCloseoutResponse closeout() {
        ...
        List<OpsShardReadinessRouteCleanupPostPushCloseoutResponse.CloseoutSignal> signals = List.of(
                ...
                signal("boundary-status", OpsShardReadinessRouteCleanupEvidenceAnalyzer.boundaryStatus()),
                signal("execution-disabled", String.valueOf(!certificate.executionAllowed()
                        && !ciEvidence.executionAllowed()))
        );
