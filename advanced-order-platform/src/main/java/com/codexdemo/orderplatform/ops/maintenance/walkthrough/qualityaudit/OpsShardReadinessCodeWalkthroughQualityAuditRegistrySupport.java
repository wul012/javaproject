  ...
  static OpsShardReadinessCodeWalkthroughQualityAuditRegistryResponse response(
    ...
    int mediumGranularityVersionCount =
        (int)
            versionCopy.stream()
                .filter(
                    OpsShardReadinessCodeWalkthroughQualityAuditRegistryResponse.VersionAudit
                        ::mediumGranularity)
                .count();
    int passedRubricScoreCount =
        (int)
            rubricCopy.stream()
                .filter(
                    OpsShardReadinessCodeWalkthroughQualityAuditRegistryResponse.RubricScore
                        ::passed)
                .count();
    int blockingReviewFindingCount =
        (int)
            findingCopy.stream()
                .filter(
                    OpsShardReadinessCodeWalkthroughQualityAuditRegistryResponse.ReviewFinding
                        ::blocking)
                .count();
    int deniedBoundaryAuditCount =
        (int) boundaryCopy.stream().filter(audit -> !audit.allowed()).count();
    boolean batchesPassed =
        batchCopy.stream()
            .allMatch(batch -> batch.standardWalkthroughs() && batch.mediumGranularity());
    boolean verificationsRequired =
        verificationCopy.stream()
            .allMatch(
                OpsShardReadinessCodeWalkthroughQualityAuditRegistryResponse.VerificationStep
                    ::required);
    boolean statusPassed =
        batchCopy.size() == EXPECTED_BATCH_ASSESSMENT_COUNT
    ...
    checks.add(
        "code-walkthrough-quality-audit-medium-version-count-" + mediumGranularityVersionCount);
    checks.add("code-walkthrough-quality-audit-rubric-score-count-" + rubricCopy.size());
    checks.add(
        "code-walkthrough-quality-audit-passed-rubric-score-count-" + passedRubricScoreCount);
    checks.add("code-walkthrough-quality-audit-review-finding-count-" + findingCopy.size());
    checks.add(
        "code-walkthrough-quality-audit-blocking-review-finding-count-"
            + blockingReviewFindingCount);
    checks.add("code-walkthrough-quality-audit-boundary-audit-count-" + boundaryCopy.size());
    checks.add(
        "code-walkthrough-quality-audit-denied-boundary-audit-count-" + deniedBoundaryAuditCount);
    checks.add("code-walkthrough-quality-audit-verification-step-count-" + verificationCopy.size());
