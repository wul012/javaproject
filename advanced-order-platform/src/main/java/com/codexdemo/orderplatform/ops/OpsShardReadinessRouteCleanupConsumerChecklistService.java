...
public class OpsShardReadinessRouteCleanupConsumerChecklistService {
    ...
    public OpsShardReadinessRouteCleanupConsumerChecklistResponse checklist() {
        ...
        OpsShardReadinessRouteCleanupContinuityReportResponse continuity = continuityReportService.report();
        List<OpsShardReadinessRouteCleanupConsumerChecklistResponse.ChecklistItem> items = List.of(
                item("packet-passed", packet.status().equals("passed"), OpsShardReadinessRouteCleanupConsumerPacketService.ENDPOINT),
                item("continuity-passed", continuity.status().equals("passed"), OpsShardReadinessRouteCleanupContinuityReportService.ENDPOINT),
                item("read-only-boundary-held", continuity.readOnlyBoundaryHeld(), "readOnlyBoundaryHeld=true"),
                item("blocked-operations-present", packet.blockedOperations().size() >= 7,
