...
class OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffControllerTests {
    ...
    void handoffRouteExposesReadOnlyArchiveHandoff() {
        ...
        var response = new OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffController(
                service()).handoff();

        assertThat(response.endpoint())
                .isEqualTo("/api/v1/ops/shard-readiness/candidate-document-material-submission-precheck-handoff");
        assertThat(response.profile())
                .isEqualTo("java-shard-readiness-candidate-document-material-submission-precheck-handoff.v1");
        assertThat(response.version()).isEqualTo("Java v1187");
        assertThat(response.readOnly()).isTrue();
        assertThat(response.executionAllowed()).isFalse();
        assertThat(response.handoffState())
                .isEqualTo("archived-read-only-material-submission-precheck-waiting-for-reviewed-real-material");
    }
