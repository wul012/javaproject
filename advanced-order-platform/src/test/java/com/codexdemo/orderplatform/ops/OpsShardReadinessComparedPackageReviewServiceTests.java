...
class OpsShardReadinessComparedPackageReviewServiceTests {
    ...
    void exposesScopedReviewSlicesWithoutWideningLocks() {
        assertSlice(new OpsShardReadinessComparedPackageReviewSourceEvidenceService().sourceEvidence(),
                "Java v1035", 3, 3, 1);
        assertSlice(new OpsShardReadinessComparedPackageReviewComparisonOutcomeService().comparisonOutcome(),
                "Java v1036", 3, 3, 1);
        assertSlice(new OpsShardReadinessComparedPackageReviewIdentityDigestService().identityDigest(),
                "Java v1037", 3, 3, 1);
        assertSlice(new OpsShardReadinessComparedPackageReviewPolicyArchiveService().policyArchive(),
                "Java v1038", 3, 3, 2);
    }

    @Test
    void handoffCloseoutKeepsFullCatalogReadOnly() {
        var response = new OpsShardReadinessComparedPackageReviewHandoffCloseoutService().handoffCloseout();
