...
class OpsShardReadinessV1ContractConsumerReadinessHandoffArchiveSlugParityTests {
  ...
  void keepsArchiveArtifactSlugsAlignedWithCatalogEvidenceJson() {
    ...
    for (OpsShardReadinessV1ContractConsumerReadinessHandoffPostHandoffEvidenceCatalog.Receipt
      ...
      String archiveStem =
          evidenceFileName.substring(0, evidenceFileName.length() - ".json".length());
      Path versionRoot = root.resolve("e").resolve(String.valueOf(receipt.version()));

      assertThat(evidencePath.getFileName().toString())
          .as(receipt.evidencePath())
          .isEqualTo(archiveStem + ".json");
      assertThat(
              evidencePath
                  .getParent()
                  .resolve(archiveStem + "-browser-snapshot.md")
                  .getFileName()
                  .toString())
          .as(receipt.evidencePath())
