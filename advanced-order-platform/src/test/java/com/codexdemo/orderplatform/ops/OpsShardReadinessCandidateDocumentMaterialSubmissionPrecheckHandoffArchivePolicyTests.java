...
class OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffArchivePolicyTests {
    ...
    void archiveHandlesMapOneToOneToSourceCheckpoints() {
        ...
        var archiveHandles = OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffArchiveCatalog.archiveHandles(
                sourcePrecheck);

        assertThat(archiveHandles).hasSize(10);
        assertThat(archiveHandles)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ArchiveHandle::checkpointCode)
                .containsExactlyElementsOf(sourcePrecheck.checkpoints().stream()
                        ...
                        .toList());
        assertThat(archiveHandles)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ArchiveHandle::reference)
    ...
    void policyLocksMapOneToOneToSourceValidators() {
        var sourcePrecheck = sourcePrecheckService().materialSubmissionPrecheck();
        var policyLocks = OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffPolicyCatalog.policyLocks(
                sourcePrecheck);

        assertThat(policyLocks).hasSize(10);
        assertThat(policyLocks)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.PolicyLock::validatorCode)
                .containsExactlyElementsOf(sourcePrecheck.validators().stream()
                        .map(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Validator::code)
                        .toList());
        assertThat(policyLocks)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.PolicyLock::enforcement)
