...
class OpsShardReadinessMinimalReadOnlyGateExecutionRendererTests {
  ...
  void rendersStableMarkdownSectionsForOperatorArchive() {
    ...
    assertThat(response.markdownSections())
        .extracting(
            OpsShardReadinessMinimalReadOnlyGateExecutionRegistryResponse.MarkdownSection::heading)
        .containsExactly(
            "Read Targets",
            "Gate Checks",
            "Boundary Rules",
            ...
            "Operator Handoff");
    assertThat(response.markdownSections().get(0).lines().get(0)).isEqualTo("read-target-count=5");
    assertThat(response.markdownSections().get(1).lines().get(0)).isEqualTo("gate-check-count=20");
