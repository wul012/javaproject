...
class OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentRendererTests {
  ...
  void markdownAndChecksDescribeSustainmentContract() {
    ...
    assertThat(response.markdownSections())
        .extracting(
            OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentResponse.MarkdownSection
                ::heading)
        .containsExactly(
            "Source Closeout",
            "Ownership Rules",
  ...
  void ciRendererKeepsCommandsOutOfStatusLogic() {
    ...
    assertThat(response.ciGates())
        .extracting(
            OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentResponse.CiGate::gate)
        .containsExactly(
