...
class OpsShardReadinessRouteCleanupFixtureCoverageIndexServiceTests {
  ...
  void indexesPostCompletionFixtureAndTestCoverage() {
    OpsShardReadinessRouteCleanupFixtureCoverageIndexResponse index =
        OpsShardReadinessRouteCleanupPostCompletionServiceFixtures.fixtureCoverageIndexService()
            .index();

    assertThat(OpsShardReadinessRouteCleanupEvidenceAnalyzer.latestJavaVersion())
        .isGreaterThanOrEqualTo(403);
    ...
    assertThat(index.coverageItems())
        ...
        .containsExactly(
            ...
            "release-bundle-test");
    assertThat(index.coverageItems())
        .allSatisfy(item -> assertThat(item.status()).isEqualTo("covered"));
