...
class OpsShardReadinessRouteCleanupMaintenanceLatestSiblingServiceTests {
  ...
  void buildsLatestSiblingMaintenanceReportFromSplitSegment() {
    ...
    assertThat(report.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-latest-sibling-report");
    assertThat(report.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-latest-sibling-report.v1");
    ...
    assertThat(report.checks())
        .contains("latest-sibling-source-node-v549-present", "latest-sibling-remains-read-only");
