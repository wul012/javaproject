...
class OpsShardReadinessRouteCleanupMaintenanceHandoffAcceptanceDigestServiceTests {
  ...
  void buildsAcceptanceDigestFromSustainmentEvidence() {
    ...
    assertThat(digest.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-handoff-acceptance-digest.v1");
    ...
    assertThat(digest.sections())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceHandoffAcceptanceDigestResponse
                    .AcceptanceSection
                ::name)
        .containsExactly(
            "owner-coverage",
            ...
            "handoff-readiness");
    assertThat(digest.sections())
        .allSatisfy(section -> assertThat(section.status()).isEqualTo("passed"));
