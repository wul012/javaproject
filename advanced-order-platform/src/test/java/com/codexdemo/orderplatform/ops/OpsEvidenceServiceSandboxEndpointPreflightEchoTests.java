...
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalSandboxEndpointCredentialResolverDecisionEchoRecords;
...
class OpsEvidenceServiceSandboxEndpointPreflightEchoTests
  ...
  void releaseApprovalRehearsalExposesSandboxEndpointHandlePreflightEchoMarker() {
    ...
    assertThat(marker.consumedByNodeSandboxEndpointHandlePreflightReviewVersion())
        .isEqualTo("Node v258");
    ...
    assertThat(marker.sourceNodeFakeTransportPacketUpstreamEchoVerificationVersion())
        .isEqualTo("Node v257");
    ...
    assertThat(marker.nextNodeSandboxEndpointHandleUpstreamEchoVerificationVersion())
        .isEqualTo("Node v259");
  ...
  void releaseApprovalRehearsalExposesSandboxEndpointCredentialResolverDecisionEchoMarker() {
    ...
    ReleaseApprovalSandboxEndpointCredentialResolverDecisionEchoRecords
        ...
        marker = rehearsal.managedAuditSandboxEndpointCredentialResolverDecisionEchoMarker();
    assertThat(marker.markerVersion())
        .isEqualTo(
    ...
    assertThat(
            marker.consumedByNodeSandboxEndpointCredentialResolverDecisionRecordMarkdownEndpoint())
        .isEqualTo(
    ...
    assertThat(marker.recordMode())
        .isEqualTo("sandbox-endpoint-credential-resolver-decision-record-only");
    assertThat(marker.sourceSpan())
        .isEqualTo("Node v259 sandbox endpoint handle upstream echo verification");
    ...
    assertThat(marker.decisionRecord().resolverMode())
        .isEqualTo("policy-record-only-no-value-read");
    assertThat(marker.decisionRecord().resolverCandidateImplementation())
    ...
    assertThat(marker.sideEffectBoundaryEchoed()).isTrue();
    assertThat(marker.readyForNodeV261SandboxEndpointCredentialResolverUpstreamEchoVerification())
        .isTrue();
    assertThat(marker.readyForManagedAuditSandboxAdapterConnection()).isFalse();
    assertThat(marker.readyForProductionAudit()).isFalse();
    assertThat(marker.readyForProductionWindow()).isFalse();
    assertThat(marker.nodeMayTreatAsProductionAuditRecord()).isFalse();
    assertThat(marker.requiredDecisionFieldIds())
            ...
            marker.decisionRecord().requiredDecisionFields().stream()
                .map(
                    ReleaseApprovalSandboxEndpointCredentialResolverDecisionEchoRecords
                        ...
                        ::id)
                .toList());
    assertThat(marker.explicitNoGoConditionCodes())
        .containsExactlyElementsOf(
