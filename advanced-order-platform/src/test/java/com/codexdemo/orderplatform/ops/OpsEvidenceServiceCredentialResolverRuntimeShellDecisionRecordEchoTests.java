...
class OpsEvidenceServiceCredentialResolverRuntimeShellDecisionRecordEchoTests
  ...
  void releaseApprovalRehearsalAddsRuntimeShellDecisionRecordEchoReceiptForNodeV300() {
    ...
    assertThat(receipt.decisionRecord().requiredEvidence())
        .allMatch(
            evidence ->
                "present".equals(evidence.status()) && evidence.requiredBeforeRuntimeShell());
    ...
    assertThat(receipt.decisionRecord().explicitNoGoConditions())
        .allMatch(
            condition -> "pause-and-do-not-implement-runtime-shell".equals(condition.action()));
    ...
    assertThat(
            receipt.checks().readyForNodeV300RuntimeShellDecisionRecordUpstreamEchoVerification())
        .isTrue();
    ...
    assertThat(receipt.readyForNodeV300RuntimeShellDecisionRecordUpstreamEchoVerification())
        .isTrue();
