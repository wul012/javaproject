...
class OpsShardReadinessRouteCleanupSourcePlanAlignmentServiceTests {
  ...
  void recordsCurrentNodePlanAlignmentWithoutStartingSiblingServices() {
    ...
    assertThat(alignment.sourcePlanPath())
        .contains("docs/plans3", "v549-post-java-mini-kv-route-catalog-cleanup");
    ...
    assertThat(alignment.alignments())
        ...
        .contains("source-plan", "sibling-startup", "runtime-boundary", "collaboration-mode");
    assertThat(alignment.alignments())
        .allSatisfy(
            item -> {
