...
import static com.codexdemo.orderplatform.ops.maintenance.v1contract.OpsShardReadinessV1ContractConsumerReadinessHandoffArchiveTestSupport.versionedArtifact;
...
class OpsShardReadinessV1ContractConsumerReadinessHandoffArchiveArtifactByteFloorTests {
  ...
  void keepsRecentArchiveArtifactsAboveByteFloor() throws IOException {
    for (int version = 275; version <= 286; version++) {
      ...
      assertThat(Files.size(versionedArtifact(version, "-browser-snapshot.md")))
          .isGreaterThan(200L);
