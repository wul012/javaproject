...
class OpsShardReadinessRouteCleanupMaintenanceFailClosedPolicyServiceTests {
  ...
  void buildsFailClosedPolicyReportFromEvidenceBoundaries() {
    ...
    assertThat(report.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-fail-closed-policy");
    assertThat(report.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-fail-closed-policy.v1");
    ...
    assertThat(report.policies())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceFailClosedPolicyResponse.PolicyCheck::operation)
        .containsExactly(
            "write-routing",
            "active-shard-router",
