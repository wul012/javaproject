...
class OpsShardReadinessRouteCleanupCompletionAuditDigestServiceTests {
    ...
    void buildsCompletionAuditDigestFromPostCompletionEvidence() {
        OpsShardReadinessRouteCleanupCompletionAuditDigestResponse digest =
                OpsShardReadinessRouteCleanupPostCompletionServiceFixtures.completionAuditDigestService().digest();

        assertThat(OpsShardReadinessRouteCleanupEvidenceAnalyzer.latestJavaVersion()).isGreaterThanOrEqualTo(405);
        assertThat(digest.project()).isEqualTo("advanced-order-platform");
        assertThat(digest.version()).isEqualTo(OpsShardReadinessRouteCleanupEvidenceAnalyzer.latestJavaVersionLabel());
