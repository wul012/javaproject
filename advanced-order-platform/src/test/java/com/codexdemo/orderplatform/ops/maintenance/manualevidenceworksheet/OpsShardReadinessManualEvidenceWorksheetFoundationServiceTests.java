...
class OpsShardReadinessManualEvidenceWorksheetFoundationServiceTests {
  ...
  void buildsWorksheetCatalogWithEntryLocks() {
    ...
    assertThat(catalog.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/manual-evidence-worksheet-catalog");
    assertThat(catalog.profile())
        .isEqualTo("java-shard-readiness-manual-evidence-worksheet-catalog.v1");
    ...
    assertThat(catalog.items())
        ...
        .containsExactly(
            "source-review-package", "blank-slot-count", "gate-count", "fail-closed-flags");
  ...
  void buildsSlotTemplateWithoutManualValues() {
    ...
    assertThat(template.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/manual-evidence-worksheet-slot-template");
    assertThat(template.profile())
        .isEqualTo("java-shard-readiness-manual-evidence-worksheet-slot-template.v1");
    ...
    assertThat(template.items())
        .extracting(OpsShardReadinessManualEvidenceWorksheetResponse.WorksheetItem::name)
        .containsExactly("slot-identity", "blank-value-state", "operator-note", "validation-hint");
    assertThat(template.checks())
        .contains(
  ...
  void buildsValidationRulesThatRejectRuntimePayloads() {
    ...
    assertThat(rules.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/manual-evidence-worksheet-validation-rules");
    assertThat(rules.profile())
        .isEqualTo("java-shard-readiness-manual-evidence-worksheet-validation-rules.v1");
  ...
  void buildsRedactionRulesThatExcludeSecretsAndRawEndpoints() {
    ...
    assertThat(rules.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/manual-evidence-worksheet-redaction-rules");
    assertThat(rules.profile())
        .isEqualTo("java-shard-readiness-manual-evidence-worksheet-redaction-rules.v1");
    assertThat(rules.readyForOperatorEntryWorksheet()).isTrue();
  ...
  void buildsMissingValuePolicyThatBlocksImporterReadiness() {
    ...
    assertThat(policy.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/manual-evidence-worksheet-missing-value-policy");
    assertThat(policy.profile())
        .isEqualTo("java-shard-readiness-manual-evidence-worksheet-missing-value-policy.v1");
  ...
  void buildsTargetScopeRegistryWithoutActivatingRouting() {
    ...
    assertThat(registry.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/manual-evidence-worksheet-target-scope-registry");
    assertThat(registry.profile())
        .isEqualTo("java-shard-readiness-manual-evidence-worksheet-target-scope-registry.v1");
