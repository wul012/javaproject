package com.codexdemo.orderplatform.ops.maintenance.overview;
...
class OpsOverviewServiceTests {
  ...
  private final InventoryRepository inventoryRepository =
      org.mockito.Mockito.mock(InventoryRepository.class);
  ...
  void aggregatesReadOnlySignalsForOpsOverview() {
    MockEnvironment environment =
        new MockEnvironment().withProperty("spring.application.name", "advanced-order-platform");
    environment.setActiveProfiles("local", "ops");
    OpsOverviewService service =
    ...
    when(failedEventMessageRepository.countByReplayApprovalStatus(
