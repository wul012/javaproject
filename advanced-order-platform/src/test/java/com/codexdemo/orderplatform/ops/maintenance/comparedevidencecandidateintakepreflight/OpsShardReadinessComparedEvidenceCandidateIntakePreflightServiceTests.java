...
class OpsShardReadinessComparedEvidenceCandidateIntakePreflightServiceTests {
  ...
  void exposesScopedCandidateIntakeSlicesWithoutRuntimeOrWrites() {
    assertSlice(
        new OpsShardReadinessComparedEvidenceCandidateIntakePreflightSourceService().source(),
        "Java v1076",
        3);
    assertSlice(
        ...
        3);
    assertSlice(
        new OpsShardReadinessComparedEvidenceCandidateIntakePreflightPolicyService().policy(),
        "Java v1078",
        2);
    assertSlice(
        new OpsShardReadinessComparedEvidenceCandidateIntakePreflightCloseoutService().closeout(),
        "Java v1079",
        2);
  }
