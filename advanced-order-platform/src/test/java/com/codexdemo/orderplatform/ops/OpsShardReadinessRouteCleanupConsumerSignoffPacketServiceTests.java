...
class OpsShardReadinessRouteCleanupConsumerSignoffPacketServiceTests {
    ...
    void buildsConsumerSignoffPacketWithoutOpeningRuntimeCapabilities() {
        OpsShardReadinessRouteCleanupConsumerSignoffPacketResponse packet =
                OpsShardReadinessRouteCleanupPostCompletionServiceFixtures.consumerSignoffPacketService().packet();

        assertThat(OpsShardReadinessRouteCleanupEvidenceAnalyzer.latestJavaVersion()).isGreaterThanOrEqualTo(397);
        assertThat(packet.project()).isEqualTo("advanced-order-platform");
        assertThat(packet.version()).isEqualTo(OpsShardReadinessRouteCleanupEvidenceAnalyzer.latestJavaVersionLabel());
