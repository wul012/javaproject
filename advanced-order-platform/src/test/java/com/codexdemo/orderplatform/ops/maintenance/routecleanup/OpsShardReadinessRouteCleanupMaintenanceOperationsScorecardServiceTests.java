...
class OpsShardReadinessRouteCleanupMaintenanceOperationsScorecardServiceTests {
  ...
  void buildsOperationsScorecardFromSustainmentServices() {
    ...
    assertThat(scorecard.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-operations-scorecard");
    assertThat(scorecard.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-operations-scorecard.v1");
    ...
    assertThat(scorecard.dimensions())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceOperationsScorecardResponse.ScorecardDimension
                ::name)
        .containsExactly(
            "handoff-acceptance", "dependency-boundary", "archive-retention", "test-evidence");
    assertThat(scorecard.dimensions())
        .allSatisfy(
            dimension -> {
