...
class OpsShardReadinessRouteCleanupMaintenanceSustainmentBatchServiceTests {
  ...
  void buildsContractFreezeFromNodeV549Plan() {
    ...
    assertThat(freeze.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-contract-freeze");
    assertThat(freeze.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-contract-freeze.v1");
    ...
    assertThat(freeze.items())
        ...
        .containsExactly(
            "read-only-integration-v1", "shard-readiness-v1", "runtime-boundary", "source-plan");
    assertThat(freeze.checks())
        .contains(
  ...
  void buildsGateHandoffWithoutRunningTestsOrStartingUpstreams() {
    ...
    assertThat(handoff.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-gate-handoff");
    ...
    assertThat(handoff.items())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceSustainmentReviewResponse.ReviewItem::name)
        .containsExactly(
            "focused-tests", "grouped-route-tests", "build-validation", "smoke-read-only");
  ...
  void buildsShardFieldMapWithoutEnablingRouting() {
    ...
    assertThat(fieldMap.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-shard-field-map");
    assertThat(fieldMap.itemCount()).isEqualTo(4);
    assertThat(fieldMap.items())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceSustainmentReviewResponse.ReviewItem::name)
        .containsExactly("project-version", "read-only-boundary", "shard-shape", "evidence-path");
    assertThat(fieldMap.checks())
        .contains(
            "shard-readiness-v1-minimal-fields-mapped", "field-map-does-not-enable-active-routing");
  ...
  void buildsReadWindowEvidenceWithoutLiveProbe() {
    ...
    assertThat(evidence.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-read-window-evidence");
    assertThat(evidence.itemCount()).isEqualTo(4);
    assertThat(evidence.items())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceSustainmentReviewResponse.ReviewItem::name)
        .containsExactly(
            "health-read", "ops-overview-read", "shard-readiness-read", "startup-boundary");
  ...
  void buildsRuntimeBoundaryChecklistWithoutExecution() {
    ...
    assertThat(checklist.checks())
        .contains("runtime-boundary-items-5", "runtime-boundary-checklist-does-not-execute");
