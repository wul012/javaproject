...
class OpsShardReadinessRouteCleanupPostPushCloseoutServiceTests {
    ...
    void closesOutCompletionCertificateAfterPushEvidence() {
        OpsShardReadinessRouteCleanupPostPushCloseoutResponse closeout =
                OpsShardReadinessRouteCleanupPostCompletionServiceFixtures.postPushCloseoutService().closeout();

        assertThat(OpsShardReadinessRouteCleanupEvidenceAnalyzer.latestJavaVersion()).isGreaterThanOrEqualTo(389);
