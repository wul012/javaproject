...
class OpsEvidenceServiceHumanApprovalArtifactReviewPostEchoDecisionGateEchoTests
  ...
  void releaseApprovalRehearsalAddsPostEchoDecisionGateEchoReceiptForNodeV311() {
    ...
    assertThat(receipt.nextNodePostEchoDecisionUpstreamEchoVerificationVersion())
        .isEqualTo("Node v311");
    ...
    assertThat(
            receipt.sourceNodeV309().readyForHumanApprovalArtifactReviewUpstreamEchoVerification())
        .isTrue();
    ...
    assertThat(receipt.decisionGate().necessityProof().stopCondition())
        .contains(
            "credential values", "runtime shell implementation or invocation", "mini-kv write");
    ...
    assertThat(
            receipt.sideEffectBoundary().humanApprovalArtifactReviewPostEchoDecisionGateEchoOnly())
        .isTrue();
