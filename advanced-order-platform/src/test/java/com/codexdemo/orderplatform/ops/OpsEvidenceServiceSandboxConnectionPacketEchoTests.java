...
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalRehearsalSandboxConnectionResponseRecords;
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalRehearsalTestSupport;
import com.codexdemo.orderplatform.ops.maintenance.releaseapproval.ReleaseApprovalSandboxConnectionAdapterPreflightEchoRecords;
...
class OpsEvidenceServiceSandboxConnectionPacketEchoTests
  ...
  void releaseApprovalRehearsalExposesDryRunCommandPackageEchoReceipt() {
    ...
    assertThat(receipt.fieldEcho().credentialHandleCommandId())
        .isEqualTo("verify-credential-handle");
    ...
    assertThat(receipt.readyForNodeV244ManualSandboxDryRunCommandUpstreamEchoVerification())
        .isTrue();
    ...
    assertThat(
            repeated.managedAuditSandboxConnectionDryRunCommandPackageEchoReceipt().receiptDigest())
  ...
  void releaseApprovalRehearsalExposesPrecheckPacketEchoReceipt() {
    ...
    assertThat(receipt.fieldEcho().ownerApprovalArtifactItemId())
        .isEqualTo("owner-approval-artifact");
    assertThat(receipt.fieldEcho().credentialHandleReviewItemId())
        .isEqualTo("credential-handle-review");
    assertThat(receipt.fieldEcho().schemaMigrationRehearsalItemId())
        .isEqualTo("schema-migration-rehearsal");
    ...
    assertThat(receipt.javaExecutionBoundary().upstreamServiceAutoStartRequestedByJava()).isFalse();
    assertThat(receipt.javaExecutionBoundary().miniKvWritePermissionRequestedByJava()).isFalse();
    assertThat(receipt.readyForNodeV246ManualSandboxConnectionPrecheckUpstreamReceiptVerification())
        .isTrue();
    assertThat(receipt.readyForManagedAuditSandboxAdapterConnection()).isFalse();
    assertThat(receipt.receiptWarnings()).isEmpty();
    assertThat(receipt.receiptDigest()).startsWith("sha256:");
  ...
  void releaseApprovalRehearsalExposesDisabledAdapterClientPrecheckEchoReceipt() {
    ...
    assertThat(receipt.javaExecutionBoundary().actualConnectionAttemptedByJava()).isFalse();
    assertThat(receipt.javaExecutionBoundary().externalManagedAuditConnectionOpenedByJava())
        .isFalse();
  ...
  void releaseApprovalRehearsalExposesFakeTransportDryRunPacketEchoMarker() {
    ...
    assertThat(marker.consumedByNodeFakeTransportPacketArchiveVerificationVersion())
        .isEqualTo("Node v256");
    ...
    assertThat(marker.nextNodeFakeTransportPacketUpstreamEchoVerificationVersion())
        .isEqualTo("Node v257");
    ...
    assertThat(marker.requestShape().requestId())
        .isEqualTo("managed-audit-v255-fake-transport-dry-run");
