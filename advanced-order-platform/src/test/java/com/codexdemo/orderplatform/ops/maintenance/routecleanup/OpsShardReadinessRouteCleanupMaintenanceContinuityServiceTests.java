...
class OpsShardReadinessRouteCleanupMaintenanceContinuityServiceTests {
  ...
  void verifiesRouteCleanupEvidenceContinuityAcrossSplitSegments() {
    ...
    assertThat(continuity.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-continuity");
    assertThat(continuity.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-continuity.v1");
    ...
    assertThat(continuity.checks())
        .contains("segment-boundaries-are-contiguous", "all-entries-remain-read-only");
