...
class OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckCatalogTests {
    ...
    void modulesRemainShortOrderedAndEvidenceOnly() {
        assertThat(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckCatalog.modules())
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.ModuleEntry::order)
                .containsExactly(209, 210, 211, 212, 213);
        assertThat(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckCatalog.modules())
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.ModuleEntry::status)
                .containsOnly("passed");
    }
    ...
    void checkpointsPartitionAllSourceRequestsAndAcceptanceChecks() {
        ...
        assertThat(flattenRequestCodes(checkpoints))
                .containsExactlyElementsOf(sourceRequest.requestItems().stream()
        ...
        assertThat(flattenAcceptanceCodes(checkpoints))
                .containsExactlyElementsOf(sourceRequest.acceptanceChecks().stream()
    ...
    void validatorsMapOneToOneToCheckpoints() {
        ...
        var validators = OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckCatalog.validators(
                checkpoints);

        assertThat(validators).hasSize(10);
        assertThat(validators)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Validator::checkpointCode)
                .containsExactlyElementsOf(checkpoints.stream()
                        .map(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.SubmissionCheckpoint::code)
                        .toList());
        assertThat(validators)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Validator::enforcement)
