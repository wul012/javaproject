...
class OpsEvidenceServiceCredentialResolverRuntimeShellPostDecisionPlanIntakeEchoTests
  ...
  void releaseApprovalRehearsalAddsRuntimeShellPostDecisionPlanIntakeEchoReceiptForNodeV302() {
    ...
    assertThat(receipt.sourceDecisionRecordEcho().readyForDisabledRuntimeShellImplementation())
        .isFalse();
    assertThat(receipt.sourceDecisionRecordEcho().readyForDisabledRuntimeShellInvocation())
        .isFalse();
    assertThat(receipt.sourceDecisionRecordEcho().readyForManagedAuditResolverImplementation())
        .isFalse();
    ...
    assertThat(receipt.necessityProof().stopCondition()).contains("credential values", "HTTP/TCP");
    assertThat(receipt.necessityProof().growthControl()).contains("After Node v302");
