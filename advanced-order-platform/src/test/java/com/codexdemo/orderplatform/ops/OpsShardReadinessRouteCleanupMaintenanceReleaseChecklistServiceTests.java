...
class OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistServiceTests {
    ...
    void buildsReleaseChecklistFromUpkeepReadinessEvidence() {
        OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistResponse checklist = service().checklist();

        assertThat(checklist.version()).isEqualTo("Java v512");
        assertThat(checklist.endpoint()).isEqualTo(
                "/api/v1/ops/shard-readiness/route-cleanup-maintenance-release-checklist");
        ...
        assertThat(checklist.items())
                .extracting(OpsShardReadinessRouteCleanupMaintenanceReleaseChecklistResponse
                        .ChecklistItem::name)
                .containsExactly(
                        "catalog-baseline",
                        "readiness-gate",
