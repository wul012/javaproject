  ...
  void listsTwentyFiveArtifactFragmentsWithoutMaterialization() {
    ...
    assertThat(fragments)
        .hasSize(
            OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalCaptureArtifactPreflightFragmentCatalog
                .FRAGMENT_COUNT);
    assertThat(fragments.stream().map(fragment -> fragment.code()).collect(Collectors.toSet()))
        .hasSize(25);
    assertThat(fragments.stream().map(fragment -> fragment.sealCode()).collect(Collectors.toSet()))
        .hasSize(25);
    assertThat(fragments)
        .allSatisfy(
            fragment -> {
              ...
              assertThat(fragment.sourceEndpoint())
                  .startsWith(
