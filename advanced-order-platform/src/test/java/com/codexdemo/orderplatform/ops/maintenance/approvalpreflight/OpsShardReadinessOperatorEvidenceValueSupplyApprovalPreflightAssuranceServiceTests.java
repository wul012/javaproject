...
class OpsShardReadinessOperatorEvidenceValueSupplyApprovalPreflightAssuranceServiceTests {
  ...
  void buildsValueRejectionWithoutAcceptingMalformedMissingOrTypedValueBodies() {
    ...
    assertThat(rejection.policies())
        .extracting(
            OpsShardReadinessOperatorEvidenceValueSupplyApprovalPreflightResponse.ApprovalPolicy
                ::code)
        .containsExactly(
  ...
  void buildsCleanupReceiptWithoutWritingFilesOrStartingProcesses() {
    ...
    assertThat(receipt.items().get(0).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_23_CLEANUP_RECEIPT_ID");
    assertThat(receipt.policies().get(0).code())
        .isEqualTo("APPROVAL_PREFLIGHT_POLICY_17_CLEANUP_RECEIPT_REQUIRED");
  ...
  void buildsImportFirewallWithoutImportRuntimeOrProductionReadiness() {
    ...
    assertThat(firewall.items().get(0).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_21_ZERO_ACCEPTED_VALUE_COUNT");
    assertThat(firewall.items().get(4).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_25_CLOSEOUT_LOCKS_HELD");
  ...
  void buildsDigestBlueprintWithoutApprovalCaptureValueHashOrImportReadiness() {
    ...
    assertThat(blueprint.items().get(0).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_01_PACKET_ID");
    assertThat(blueprint.items().get(24).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_25_CLOSEOUT_LOCKS_HELD");
  ...
  void buildsArchivePlanWithoutWritingFilesStartingProcessesOrImportingValues() {
    ...
    assertThat(plan.items().get(0).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_21_ZERO_ACCEPTED_VALUE_COUNT");
    assertThat(plan.items().get(4).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_25_CLOSEOUT_LOCKS_HELD");
  ...
  void buildsCloseoutWithAllApprovalPreflightLocksHeld() {
    ...
    assertThat(closeout.items().get(0).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_01_PACKET_ID");
    assertThat(closeout.items().get(24).code())
        .isEqualTo("VALUE_SUPPLY_APPROVAL_PACKET_25_CLOSEOUT_LOCKS_HELD");
