...
class OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftReadinessOwnershipCatalogTests {
  ...
  void listsOwnershipRulesForReadinessAndClosedExecutionBoundaries() {
    ...
    assertThat(ownershipRules)
        .hasSize(
            OpsShardReadinessOperatorEvidenceValueSupplySignedApprovalArtifactDraftReadinessOwnershipCatalog
                .OWNERSHIP_COUNT);
    assertThat(ownershipRules.stream().map(rule -> rule.code()).collect(Collectors.toSet()))
        .hasSize(20);
    assertThat(ownershipRules)
        .allSatisfy(
            rule -> {
    ...
    assertThat(ownershipRules)
        .anySatisfy(rule -> assertThat(rule.code()).contains("NO_ARTIFACT_DRAFT"));
    assertThat(ownershipRules)
        .anySatisfy(rule -> assertThat(rule.code()).contains("NO_SIBLING_MUTATION"));
  }
}
