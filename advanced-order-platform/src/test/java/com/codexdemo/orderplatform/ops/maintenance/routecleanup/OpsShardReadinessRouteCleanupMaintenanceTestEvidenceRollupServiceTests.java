...
class OpsShardReadinessRouteCleanupMaintenanceTestEvidenceRollupServiceTests {
  ...
  void buildsTestEvidenceRollupForSustainmentRoutes() {
    ...
    assertThat(rollup.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-test-evidence-rollup");
    assertThat(rollup.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-test-evidence-rollup.v1");
    assertThat(rollup.evidenceEntryCount()).isEqualTo(5);
    assertThat(rollup.coveredEntryCount()).isEqualTo(5);
    assertThat(rollup.entries())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceTestEvidenceRollupResponse.TestEvidenceEntry
                ::coverageType)
    ...
    assertThat(rollup.entries())
        .allSatisfy(entry -> assertThat(entry.status()).isEqualTo("covered"));
