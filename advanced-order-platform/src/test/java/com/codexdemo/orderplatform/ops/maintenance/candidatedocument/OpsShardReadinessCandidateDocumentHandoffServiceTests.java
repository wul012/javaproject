...
class OpsShardReadinessCandidateDocumentHandoffServiceTests {
  ...
  void buildsReadOnlyCandidateDocumentHandoffFromRequestPackage() {
    ...
    assertThat(response.sourceRequestPackageEndpoint())
  ...
  void artifactHandlesMirrorEveryRequestPackageItemWithoutImportingDocuments() {
    ...
    assertThat(response.artifactHandles())
        .allSatisfy(
            handle -> {
              assertThat(handle.evidenceRef())
                  .startsWith("candidate-document-request-package/evidence/");
              assertThat(handle.digestRef()).endsWith(".sha256");
              assertThat(handle.archiveRef())
                  .startsWith("candidate-document-request-package/archive/");
  ...
  void artifactHandlePathsUseStableLowercaseSlugs() {
    ...
    assertThat(response.artifactHandles())
        .allSatisfy(
            handle -> {
              assertThat(handle.evidenceRef())
                  .matches("candidate-document-request-package/evidence/[a-z0-9-]+\\.json");
              assertThat(handle.digestRef())
                  .matches("candidate-document-request-package/digests/[a-z0-9-]+\\.sha256");
              assertThat(handle.archiveRef())
                  .matches("candidate-document-request-package/archive/[a-z0-9-]+\\.md");
            });
  }
  ...
  void gatesAndChecksRecordCountAndReadOnlyBoundaries() {
    ...
    assertThat(response.gates()).last().isEqualTo("candidate-document-handoff-read-only-gate-25");
