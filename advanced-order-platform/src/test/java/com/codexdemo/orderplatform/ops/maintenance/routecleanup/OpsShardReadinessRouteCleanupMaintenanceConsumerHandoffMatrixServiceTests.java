...
class OpsShardReadinessRouteCleanupMaintenanceConsumerHandoffMatrixServiceTests {
  ...
  void buildsConsumerHandoffMatrixFromUpkeepCatalog() {
    ...
    assertThat(matrix.endpoint())
        .isEqualTo("/api/v1/ops/shard-readiness/route-cleanup-maintenance-consumer-handoff-matrix");
    assertThat(matrix.profile())
        .isEqualTo("java-shard-readiness-route-cleanup-maintenance-consumer-handoff-matrix.v1");
    ...
    assertThat(matrix.matrix())
        .extracting(
            OpsShardReadinessRouteCleanupMaintenanceConsumerHandoffMatrixResponse.MatrixEntry
                ::consumer)
        .contains("catalog-maintainer", "runtime-boundary-reviewer", "operator-handoff-reviewer");
