...
class OpsShardReadinessRouteCleanupCiRunAttestationServiceTests {
    ...
    void buildsReadOnlyCiRunAttestationFromPostPushCloseout() {
        OpsShardReadinessRouteCleanupCiRunAttestationResponse attestation =
                OpsShardReadinessRouteCleanupPostCompletionServiceFixtures.ciRunAttestationService().attestation();

        assertThat(OpsShardReadinessRouteCleanupEvidenceAnalyzer.latestJavaVersion()).isGreaterThanOrEqualTo(391);
