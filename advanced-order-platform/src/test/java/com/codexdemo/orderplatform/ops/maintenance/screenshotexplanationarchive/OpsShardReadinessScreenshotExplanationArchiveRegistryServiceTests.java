...
class OpsShardReadinessScreenshotExplanationArchiveRegistryServiceTests {
  ...
  void buildsScreenshotExplanationArchiveSegmentationRegistry() {
    ...
    assertThat(response.sourcePlan()).isEqualTo("Node v367 / Java v1769-v1773");
    ...
    assertThat(response.segmentPlanCount()).isEqualTo(5);
  ...
  void namesTheCurrentAndNextScreenshotArchiveRoots() {
    ...
    assertThat(response.currentArchiveAssessments())
        .extracting(
            OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.CurrentArchiveAssessment
                ::root)
        .containsExactly("d", "d_runtime_screenshot_archive_next", "f");
    assertThat(response.segmentPlans())
        .extracting(
            OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.ArchiveSegmentPlan::path)
        .contains(
    ...
    assertThat(response.namingRules())
        .extracting(OpsShardReadinessScreenshotExplanationArchiveRegistryResponse.NamingRule::code)
        .contains("range-before-version", "separate-images-and-explanations", "no-root-dumping");
  }
}
