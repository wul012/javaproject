...
class OpsShardReadinessComparedEvidenceEvaluationPreflightCatalogTests {
  ...
  void catalogListsTwentyRulesAndTwentyFailClosedGuards() {
    var response =
        new OpsShardReadinessComparedEvidenceEvaluationPreflightCatalogService().catalog();
  ...
  void evaluationRulesKeepNodeVersionContinuityAndUniqueCodes() {
    ...
    assertThat(rules)
        .extracting(
            OpsShardReadinessComparedEvidenceEvaluationPreflightResponse.EvaluationRule::code)
        .doesNotHaveDuplicates()
        .contains("source-intake-readiness", "evaluation-closeout");
    assertThat(rules)
        .extracting(
            OpsShardReadinessComparedEvidenceEvaluationPreflightResponse.EvaluationRule
                ::sourceNodeVersion)
