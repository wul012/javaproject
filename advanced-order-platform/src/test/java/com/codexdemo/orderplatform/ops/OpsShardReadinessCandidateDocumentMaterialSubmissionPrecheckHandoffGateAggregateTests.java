...
class OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffGateAggregateTests {
    ...
    void gateCatalogIsDistinctAndMatchesAggregateCount() {
        var handoff = OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffTestSupport.handoff();

        assertThat(handoff.gates())
                .hasSize(42)
                .doesNotHaveDuplicates()
                .allSatisfy(gate -> assertThat(gate)
                        .startsWith("candidate-document-material-submission-precheck-handoff-no-material-gate-"));
