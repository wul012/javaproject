...
final class OpsEvidenceServiceEndpointAssertions {

    private OpsEvidenceServiceEndpointAssertions() {
    }

    static void assertAdditionalProbeEndpoints(OpsEvidenceResponse evidence) {
        assertThat(evidence.healthProbe().additionalProbeEndpoints())
                .containsExactly(additionalProbeEndpoints());
    }

    static void assertEvidenceEndpoints(OpsEvidenceResponse evidence) {
        assertThat(evidence.evidenceEndpoints())
                .contains(evidenceEndpoints());
    }

    private static String[] additionalProbeEndpoints() {
        return Stream.concat(
                        Stream.of("/api/v1/ops/overview"),
                        sharedShardReadinessAndContractEndpoints()
                )
                .toArray(String[]::new);
    }

    private static String[] evidenceEndpoints() {
        return Stream.concat(
                        Stream.concat(
                                Stream.of("/api/v1/ops/evidence"),
                                sharedShardReadinessAndContractEndpoints()
                        ),
                        Stream.of(
    ...
    private static Stream<String> sharedShardReadinessAndContractEndpoints() {
        return Stream.of(
                ...
                "/api/v1/ops/shard-readiness/v1-contract-consumer-readiness-handoff",
