...
class OpsShardReadinessReleaseAcceptanceRoutePathSplitSustainmentAcceptancePackageCloseoutArchiveIndexCatalogTests {
    ...
    void archiveIndexEvidenceIsReady() {
        ...
        assertThat(response.verificationGates())
                .allSatisfy(gate -> assertThat(gate.passed()).isTrue());
        assertThat(response.handoffNotes())
                .allSatisfy(note -> assertThat(note.ready()).isTrue());
    }
}
