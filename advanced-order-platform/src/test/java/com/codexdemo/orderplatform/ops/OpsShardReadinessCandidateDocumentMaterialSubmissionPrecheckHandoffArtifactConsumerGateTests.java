...
class OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffArtifactConsumerGateTests {
    ...
    void artifactReferencesMirrorSourceArtifactsIntoHandoffArchive() {
        ...
        var references = OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffArtifactCatalog.artifactReferences(
                sourcePrecheck);

        assertThat(references).hasSize(8);
        assertThat(references)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ArtifactReference::sourceReference)
                .containsExactlyElementsOf(sourcePrecheck.artifacts().stream()
                        .map(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckResponse.Artifact::reference)
                        .toList());
        assertThat(references)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ArtifactReference::archiveReference)
                .allSatisfy(reference -> assertThat(reference).startsWith("e/1187/artifacts/"));
    }

    @Test
    void consumerRulesAllowOnlyArchiveReads() {
        var rules = OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffConsumerCatalog.consumerRules(
                sourcePrecheckService().materialSubmissionPrecheck());

        assertThat(rules).hasSize(10);
        assertThat(rules)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ConsumerRule::allowedAction)
                .containsOnly("read archive handle and policy lock");
        assertThat(rules)
                .extracting(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffResponse.ConsumerRule::blockedAction)
                .allSatisfy(action -> assertThat(action).contains("submit", "import", "write"));
    }

    @Test
    void handoffGatesAddOneCloseoutGateAfterSourcePrecheck() {
        assertThat(OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckHandoffGateCatalog.gates())
                .hasSize(42)
                .first()
    ...
    private OpsShardReadinessCandidateDocumentMaterialSubmissionPrecheckService sourcePrecheckService() {
