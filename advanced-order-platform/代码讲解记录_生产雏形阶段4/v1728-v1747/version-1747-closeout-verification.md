# Java v1747 代码讲解：closeout verification

本版目标是收尾本 20 版，运行 targeted Maven tests 并准备推送 tags。
...
入口仍是 code walkthrough compliance registry endpoint 和本目录 README。

## 响应模型

最终 response version 固定为 Java v1747，status=passed，checks 包含版本范围、标题数量、边界拒绝和 legacy cutoff。
...
先静态组装证据，再运行测试，再按 v1728-v1747 切 tag 推送。

## Java 证据检查

Java 证据检查 targeted tests 和讲解合规测试同时通过。
...
本批 targeted Maven 测试包含 code walkthrough compliance registry tests 与 OpsCodeWalkthroughArchiveComplianceTests。 本批收尾还会运行 OpsCodeWalkthroughArchiveComplianceTests，确认 v1728-v1747 的新讲解全部包含九个 required headings，且没有使用 legacy marker。

## 一句话总结

Java v1747 把收尾本 20 版，运行 targeted Maven tests 并准备推送 tags。同时保持只读证据边界关闭。
