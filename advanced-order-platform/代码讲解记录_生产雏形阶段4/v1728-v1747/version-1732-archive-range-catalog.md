# Java v1732 代码讲解：archive range catalog

本版目标是新增归档范围 catalog，把历史目录、拆分目录和当前续写目录统一列清。
...
入口是 OpsShardReadinessCodeWalkthroughComplianceArchiveRangeCatalog.archiveRanges()。

## 响应模型

ArchiveRange 记录 name、directory、versionRange、retentionRule，用于解释哪些目录是历史、哪些是新标准目录。
...
service 聚合 archive ranges，renderer 输出 Archive Ranges section。

## Java 证据检查

Java 证据检查 archiveRangeCount=5，并包含 代码讲解记录_生产雏形阶段4/v1728-v1747。
...
service tests 验证 archiveRangeCount，renderer tests 验证 section 输出。 本批收尾还会运行 OpsCodeWalkthroughArchiveComplianceTests，确认 v1728-v1747 的新讲解全部包含九个 required headings，且没有使用 legacy marker。

## 一句话总结

Java v1732 把新增归档范围 catalog，把历史目录、拆分目录和当前续写目录统一列清。同时保持只读证据边界关闭。
