# Java v1741 代码讲解：boundary tests

本版目标是补强运行边界测试，确保讲解合规 registry 不误开执行能力。
...
测试入口是 OpsShardReadinessCodeWalkthroughComplianceRegistryBoundaryTests。

## 响应模型

测试检查 readOnly=true，以及 executionAllowed、startsJavaService、startsMiniKvService、readsCredentialValue、resolvesRawEndpointUrl、managedAuditHttpAllowed 全为 false。
...
response checks 中同步输出 no-write-routing、no-active-shard-router、no-credential-value、no-raw-endpoint-url、no-managed-audit-connection。

## Java 证据检查

Java 证据检查 deniedBoundaryRuleCount == boundaryRuleCount。
...
OpsShardReadinessCodeWalkthroughComplianceRegistryBoundaryTests 两个用例覆盖。 本批收尾还会运行 OpsCodeWalkthroughArchiveComplianceTests，确认 v1728-v1747 的新讲解全部包含九个 required headings，且没有使用 legacy marker。

## 一句话总结

Java v1741 把补强运行边界测试，确保讲解合规 registry 不误开执行能力。同时保持只读证据边界关闭。
